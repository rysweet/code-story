...
class Neo4jBenchmark:
    ...
    def teardown(self) -> None:
        ...
        # Drop indexes
        self.connector.execute_query(
            "DROP INDEX test_node_id IF EXISTS",
            write=True
        )
        
        self.connector.execute_query(
            "DROP INDEX test_node_name IF EXISTS",
            write=True
        )
    ...
    def benchmark_sync_transaction(self, batch_size: int = 10) -> Dict[str, Any]:
        ...
        print(f"Benchmarking synchronous transactions ({self.iterations} iterations, batch size: {batch_size})...")
        ...
        stats["operations_per_second"] = stats["operations_per_transaction"] / stats["mean"]
    ...
    async def benchmark_concurrent_reads(self) -> Dict[str, Any]:
        ...
        print(f"Benchmarking concurrent reads ({self.iterations} iterations, concurrency: {self.concurrency})...")
    ...
    async def benchmark_async_transaction(self, batch_size: int = 10) -> Dict[str, Any]:
        ...
        print(f"Benchmarking async transactions ({self.iterations} iterations, batch size: {batch_size})...")
    ...
    def _print_summary(self) -> None:
        ...
        headers = ["Test", "Mean (s)", "Median (s)", "Min (s)", "Max (s)", "P95 (s)", "Ops/sec"]
        row_format = "{:<20} {:<10.4f} {:<10.4f} {:<10.4f} {:<10.4f} {:<10.4f} {:<10.2f}"
        
        print(" ".join(f"{h:<10}" for h in headers))
        print("-" * 80)
        
        for test_name, results in sorted(self.results.items(), key=lambda x: x[1]["mean"]):
...
async def main() -> None:
    ...
    parser.add_argument("--skip-teardown", action="store_true", help="Skip database teardown")
