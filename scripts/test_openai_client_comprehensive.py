...
def test_chat_completion(client: OpenAIClient) -> None:
    ...
    messages = [
        ...
        ChatMessage(role=ChatRole.USER, content="What is the capital of France? Answer in one word.")
    ]
    ...
    assert result.choices[0].message.content.strip().lower() == "paris", "Expected Paris as the answer"
...
async def test_async_chat(client: OpenAIClient) -> None:
    ...
    messages = [
        ChatMessage(role=ChatRole.SYSTEM, content="You are a helpful assistant."),
        ChatMessage(role=ChatRole.USER, content="What is the capital of Germany? Answer in one word.")
    ]
    ...
    assert result.choices[0].message.content.strip().lower() == "berlin", "Expected Berlin as the answer"
