...
def get_running_containers() -> list[str]:
    ...
    try:
        ...
        logger.info(
            f"Found {len(code_story_containers)} Code Story containers: {code_story_containers}"
        )
...
def inject_tokens_into_container(container_name: str) -> bool:
    ...
    try:
        ...
        if appuser_result.returncode != 0:
            logger.warning(
                f"Non-critical error copying to appuser directory: {appuser_result.stderr}"
            )
...
def authenticate_with_azure(tenant_id: str | None = None) -> bool:
    ...
    try:
        ...
        try:
            ...
            version = (
                az_version.stdout.splitlines()[0]
