...
def check_health_api_for_auth_issues() -> tuple[bool, str | None]:
    ...
    try:
        ...
        try:
            ...
            if (
                "components" in health_data and 
                "openai" in health_data["components"] and
                health_data["components"]["openai"]["status"] == "unhealthy"
            ):
...
def extract_tenant_id_from_config() -> str | None:
    ...
    try:
        ...
        cmd = [
            "docker", "exec", SERVICE_CONTAINER,
            "bash", "-c", "grep -r AZURE_TENANT_ID /app/.env* 2>/dev/null || echo ''"
        ]
        try:
            result = subprocess.run(cmd, check=True, capture_output=True, text=True)
            if result.stdout:
...
def get_running_containers() -> list[str]:
    ...
    try:
        ...
        for container in containers:
            if container and any(name in container for name in [
                "code-story-service",
                "code-story-worker",
                "code-story-mcp",
                "codestory-service",
...
def main() -> int:
    ...
    try:
        ...
        if args.check:
            ...
            if auth_failure:
                logger.info(f"Azure authentication issues detected. Tenant ID: {tenant_id or 'Unknown'}")
                return 1
            else:
