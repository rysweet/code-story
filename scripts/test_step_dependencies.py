...
def test_summarizer_dependencies():
    ...
    try:
        ...
        # Check order
        assert mocks["execution_order"].index("filesystem") < mocks["execution_order"].index("blarify"), \
            "FileSystemStep should execute before BlarifyStep"
        assert mocks["execution_order"].index("blarify") < mocks["execution_order"].index("summarizer"), \
...
def test_documentation_grapher_dependencies():
    ...
    try:
        ...
        print(f"Execution order: {mocks['execution_order']}")
        assert "filesystem" in mocks["execution_order"], "FileSystemStep should be executed"
        assert "documentation_grapher" in mocks["execution_order"], "DocumentationGrapherStep should be executed"
        assert "blarify" not in mocks["execution_order"], "BlarifyStep should not be executed"
        
        # Check order
        assert mocks["execution_order"].index("filesystem") < mocks["execution_order"].index("documentation_grapher"), \
...
def test_error_handling_in_dependency_chain():
    ...
    try:
        ...
        assert mocks["blarify"].run.call_count == 0, "BlarifyStep should not be called if dependency fails"
