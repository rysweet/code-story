...
| As a developer, I want to start, stop, cancel, and manage the status of ingestion jobs via the REST API so that I can control ingestion workflows. | • The REST API provides endpoints to start, stop, cancel, and check the status of ingestion jobs.<br>• Job statuses and progress are accurately reported.<br>• Users can cancel long-running tasks via API/CLI. |
| As a developer, I want to receive real-time ingestion progress updates so I can monitor long-running jobs. | • The WebSocket endpoint `/ws/status/{job_id}` streams progress events.<br>• Progress events include step name, completion percentage, status messages, and resource usage (`cpu_percent`, `memory_mb`).<br>• Progress and resource usage are reported for each job. |
| As a developer, I want to schedule ingestion jobs, set task priorities, and define dependencies so that I can optimize workflow execution. | • API/CLI supports scheduling jobs for future execution.<br>• Priority parameter is accepted and high-priority tasks are processed first.<br>• Task dependencies are respected in execution order.<br>• Resource usage is controlled and monitored. |
| As a developer, I want retry and failure recovery for ingestion jobs so that transient errors do not require manual intervention. | • Failed jobs can be retried individually.<br>• Retry/failure info is exposed via API.<br>• Retry/backoff policies are configurable. |
| As a developer, I want performance monitoring and metrics for ingestion jobs so that I can optimize and troubleshoot the system. | • Task execution metrics (duration, resource usage: `cpu_percent`, `memory_mb`) are collected and exposed via API and/or Prometheus.<br>• Performance metrics are available for monitoring and optimization. |
