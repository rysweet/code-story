...
def test_databases() -> Generator[dict[str, Any], None, None]:
    ...
    try:
        result = subprocess.run(['docker-compose', '-f', 'docker-compose.test.yml', 'up', '-d', 'neo4j', 'redis'], capture_output=True, text=True, check=True, timeout=180)
        print(f'Databases started: {result.stdout}')
        print('Waiting for databases to be ready...')
        _wait_for_neo4j()
        _wait_for_redis()
        db_info = {'neo4j_uri': 'bolt://localhost:7688', 'neo4j_username': 'neo4j', 'neo4j_password': 'password', 'neo4j_database': 'testdb', 'redis_url': 'redis://localhost:6380/0'}
        ...
        subprocess.run(['docker-compose', '-f', 'docker-compose.test.yml', 'down'], capture_output=True, text=True)
...
def neo4j_connector(test_databases: Any) -> None:
    ...
    connector = Neo4jConnector(uri=db_info['neo4j_uri'], username=db_info['neo4j_username'], password=db_info['neo4j_password'], database=db_info['neo4j_database'])
    try:
