...
class TestEntraValidator:
    ...
    def test_init(self: Any, mock_jwks_client: Any) -> None:
        ...
        mock_jwks_client.assert_called_once_with('https://login.microsoftonline.com/test-tenant/discovery/v2.0/keys')

    @pytest.mark.asyncio
    async def test_validate_token_success(self: Any, validator: Any, mock_jwt: Any) -> None:
        """Test successful token validation."""
        mock_key = mock.Mock()
        validator.jwks_client.get_signing_key_from_jwt.return_value = mock_key
        mock_jwt.decode.return_value = {'sub': 'test-user', 'azp': 'test-client', 'scp': 'code-story.read code-story.query'}
        with mock.patch.object(validator, '_verify_scopes') as mock_verify:
            ...
            mock_jwt.decode.assert_called_once_with('test-token', mock_key.key, algorithms=['RS256'], audience='test-audience', options={'verify_signature': True})
            mock_verify.assert_called_once_with(mock_jwt.decode.return_value)
            assert claims == mock_jwt.decode.return_value

    @pytest.mark.asyncio
    async def test_validate_token_jwt_error(self: Any, validator: Any, mock_jwt: Any) -> None:
    ...
    def test_verify_scopes_success(self: Any, validator: Any) -> None:
        ...
        with mock.patch.object(validator.scope_manager, 'has_required_scope', return_value=True) as mock_has_scope:
