...
class TestGraphServiceAdapter:
    ...
    async def test_search_success(self: Any, adapter: Any, mock_client: Any) -> None:
        ...
        response.json.return_value = {'data': [{'id': 'node-123', 'type': 'Class', 'properties': {'name': 'TestClass', 'path': '/path/to/test.py'}, 'score': 0.95}, {'id': 'node-456', 'type': 'Function', 'properties': {'name': 'testFunction'}, 'score': 0.85}]}
        future = asyncio.Future()
        future.set_result(response)
        mock_client.post.return_value = future
        results = await adapter.search(query='test', node_types=['Class', 'Function'], limit=10)
        mock_client.post.assert_called_once_with('/v1/query/search', json={'query': 'test', 'node_types': ['Class', 'Function'], 'limit': 10})
    ...
    async def test_find_node_success(self: Any, adapter: Any, mock_client: Any) -> None:
        ...
        response = mock.Mock()
        response.status_code = 200
        response.json.return_value = {'data': {'id': 'node-123', 'type': 'Class', 'properties': {'name': 'TestClass', 'path': '/path/to/test.py', 'content': 'class TestClass:\n    pass'}}}
        future = asyncio.Future()
        future.set_result(response)
    ...
    async def test_find_paths_success(self: Any, adapter: Any, mock_client: Any) -> None:
        ...
        response = mock.Mock()
        response.status_code = 200
        response.json.return_value = {'data': [{'elements': [{'element_type': 'node', 'id': 'node-123', 'type': 'Class', 'properties': {'name': 'TestClass'}}, {'element_type': 'relationship', 'id': 'rel-123', 'type': 'CALLS', 'start_node_id': 'node-123', 'end_node_id': 'node-456', 'properties': {}}, {'element_type': 'node', 'id': 'node-456', 'type': 'Function', 'properties': {'name': 'testFunction'}}]}]}
        future = asyncio.Future()
        future.set_result(response)
        mock_client.post.return_value = future
        paths = await adapter.find_paths(from_id='node-123', to_id='node-456', max_paths=3)
        mock_client.post.assert_called_once_with('/v1/query/paths', json={'from_id': 'node-123', 'to_id': 'node-456', 'max_paths': 3})
        assert len(paths) == 1
...
class TestOpenAIServiceAdapter:
    ...
    async def test_generate_code_summary_success(self: Any, adapter: Any, mock_client: Any, mock_metrics: Any) -> None:
        ...
        summary = await adapter.generate_code_summary(code='class TestClass:\n    pass', context='Test context', max_tokens=500)
    ...
    async def test_generate_path_explanation_success(self: Any, adapter: Any, mock_client: Any, mock_metrics: Any) -> None:
        ...
        future.set_result(response)
        mock_client.create_chat_completion.return_value = future
        path_elements = [{'element_type': 'node', 'id': 'node-123', 'type': 'Class', 'name': 'TestClass', 'content': 'class TestClass:\n    pass'}, {'element_type': 'relationship', 'id': 'rel-123', 'type': 'CALLS'}, {'element_type': 'node', 'id': 'node-456', 'type': 'Method', 'name': 'testMethod', 'content': 'def testMethod():\n    pass'}]
        explanation = await adapter.generate_path_explanation(path_elements=path_elements, max_tokens=300)
        mock_client.create_chat_completion.assert_called_once()
        request = mock_client.create_chat_completion.call_args[0][0]
    ...
    async def test_find_similar_code_success(self: Any, adapter: Any, mock_client: Any, mock_metrics: Any) -> None:
        ...
        results = await adapter.find_similar_code(code='def test(): pass', limit=5)
