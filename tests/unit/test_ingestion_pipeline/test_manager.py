...
def mock_prometheus_metrics() -> None:
    ...
    with patch('prometheus_client.Counter') as mock_counter, patch('prometheus_client.Gauge') as mock_gauge, patch('prometheus_client.Histogram') as mock_histogram, patch('prometheus_client.REGISTRY._names_to_collectors', {}):
...
class TestPipelineManager:
    ...
    def config_file(self: Any) -> None:
        ...
        config = {'steps': [{'name': 'filesystem'}, {'name': 'blarify'}], 'retry': {'max_retries': 3, 'back_off_seconds': 10}}
    ...
    def test_get_step_class_found(self: Any, config_file: Any, mock_step_class: Any) -> None:
        """Test getting a step class that exists."""
        with patch('codestory.ingestion_pipeline.manager.discover_pipeline_steps') as mock_discover, patch('codestory.ingestion_pipeline.manager.find_step_manually') as mock_find:
    ...
    def test_validate_steps_all_found(self: Any, config_file: Any, mock_step_class: Any) -> None:
        """Test validating steps when all are found."""
        with patch('codestory.ingestion_pipeline.manager.discover_pipeline_steps') as mock_discover:
            mock_discover.return_value = {'filesystem': mock_step_class, 'blarify': mock_step_class}
            manager = PipelineManager(config_path=config_file)
            manager._validate_steps()

    def test_validate_steps_missing(self: Any, config_file: Any, mock_step_class: Any) -> None:
        ...
        with patch('codestory.ingestion_pipeline.manager.discover_pipeline_steps') as mock_discover:
            ...
            with patch('codestory.ingestion_pipeline.manager.find_step_manually') as mock_find:
                mock_find.return_value = None
                manager = PipelineManager(config_path=config_file)
                with patch.object(manager, '_get_step_class', side_effect=lambda name: mock_step_class if name == 'filesystem' else None):
    ...
    def test_start_job(self: Any, config_file: Any, mock_step_class: Any) -> None:
        ...
        with patch('codestory.ingestion_pipeline.manager.discover_pipeline_steps') as mock_discover:
            ...
            with patch('uuid.uuid4') as mock_uuid:
                ...
                with patch('codestory.ingestion_pipeline.manager.orchestrate_pipeline') as mock_task:
    ...
    def test_status(self: Any, config_file: Any, mock_step_class: Any) -> None:
        ...
        with patch('codestory.ingestion_pipeline.manager.discover_pipeline_steps') as mock_discover:
            ...
            with patch('codestory.ingestion_pipeline.manager.get_job_status') as mock_task:
                ...
                manager = PipelineManager(config_path=config_file)
                job_id = 'test-job-id'
                manager.active_jobs[job_id] = {'task_id': 'test-task-id', 'repository_path': '/test/repo', 'start_time': 123456789, 'status': StepStatus.RUNNING}
    ...
    def test_stop(self: Any, config_file: Any, mock_step_class: Any) -> None:
        ...
        with patch('codestory.ingestion_pipeline.manager.discover_pipeline_steps') as mock_discover:
            ...
            with patch('codestory.ingestion_pipeline.manager.stop_job') as mock_task:
                mock_result = MagicMock()
                mock_result.get.return_value = {'status': StepStatus.STOPPED, 'message': 'Job has been stopped'}
                mock_task.apply_async.return_value = mock_result
                manager = PipelineManager(config_path=config_file)
                job_id = 'test-job-id'
                manager.active_jobs[job_id] = {'task_id': 'test-task-id', 'repository_path': '/test/repo', 'start_time': 123456789, 'status': StepStatus.RUNNING}
