...
class TestAuthModels:
    ...
    def test_token_response(self: Any) -> None:
        ...
        token = TokenResponse(access_token='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...', token_type='Bearer', expires_in=3600, scope='api')
    ...
    def test_user_info(self: Any) -> None:
        """Test UserInfo model."""
        user = UserInfo(id='user123', name='Test User', email='test@example.com', roles=['admin', 'user'], is_authenticated=True)
...
class TestConfigModels:
    ...
    def test_config_patch_validation(self: Any) -> None:
        ...
        valid_patch = ConfigPatch(items=[{'key': 'general.debug', 'value': True}, {'key': 'service.title', 'value': 'New Title'}], comment='Update settings')
...
class TestGraphModels:
    ...
    def test_vector_query_validation(self: Any) -> None:
        ...
        valid_query = VectorQuery(query='Find functions related to authentication', limit=20, min_score=0.6)
...
class TestIngestionModels:
    ...
    def test_ingestion_started(self: Any) -> None:
        ...
        started = IngestionStarted(job_id='job123', status=JobStatus.PENDING, source='/path/to/repo', steps=['filesystem', 'summarizer', 'docgrapher'], message='Job submitted successfully', eta=1620000000)
