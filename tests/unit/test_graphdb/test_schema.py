...
def test_get_all_schema_elements():
    ...
    assert any("Documentation" in idx and "embedding" in idx for idx in schema_elements["vector_indexes"])
...
def test_initialize_schema():
    ...
    with patch('src.codestory.graphdb.schema.get_schema_initialization_queries') as mock_get_queries:
