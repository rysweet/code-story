...
class TestOpenAIClient:
    ...
    def test_init_missing_credentials(self):
        ...
        with patch("src.codestory.llm.client.get_settings", side_effect=Exception("No settings")), \
             patch("src.codestory.llm.client.DefaultAzureCredential"), \
             patch("src.codestory.llm.client.get_bearer_token_provider"):
    ...
    def test_embed(self, client):
        ...
        with patch.object(client._sync_client.embeddings, "create") as mock_create:
            ...
            mock_create.assert_called_once()
            call_args = mock_create.call_args[1]
            assert call_args["deployment_name"] == "text-embedding-3-small"  # Default embedding model
    ...
    def test_error_handling_complete(self):
        ...
        with patch("openai.AzureOpenAI") as MockAzureOpenAI, \
            ...
            MockAzureOpenAI.return_value = MagicMock(completions=mock_completions)
    ...
    async def test_complete_async(self, client):
        ...
        with patch("src.codestory.llm.backoff.retry_on_openai_errors_async", 
