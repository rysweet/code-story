...
def client() -> None:
    ...
    with patch("openai.AzureOpenAI"), patch("openai.AsyncAzureOpenAI"), patch(
        "codestory.llm.client.DefaultAzureCredential"
    ), patch("codestory.llm.client.get_bearer_token_provider"), patch(
        ...
        "codestory.llm.client.retry_on_openai_errors_async", lambda **kw: lambda f: f
    ):
        client = OpenAIClient(
...
class TestReasoningModelSupport:
    ...
    def test_adjust_params_for_reasoning_model_o1(self: Any, client: Any) -> None:
        ...
        adjusted_params = client._adjust_params_for_reasoning_model(
            original_params, "o1"
        )
    ...
    def test_adjust_params_for_reasoning_model_no_max_tokens(
        self: Any, client: Any
    ) -> None:
    ...
    def test_adjust_params_for_reasoning_model_none_max_tokens(
        self: Any, client: Any
    ) -> None:
        """Test parameter adjustment when max_tokens is None."""
        original_params = {"max_tokens": None, "temperature": 0.3}
        adjusted_params = client._adjust_params_for_reasoning_model(
            original_params, "o1-mini"
        )
        assert "max_tokens" not in adjusted_params
        assert "max_completion_tokens" not in adjusted_params
        assert "temperature" not in adjusted_params

    def test_chat_with_reasoning_model_parameter_adjustment(
        self: Any, client: Any
    ) -> None:
        ...
        with patch.object(
            ...
            result = client.chat(
                messages=messages, model="o1", max_tokens=10, temperature=0.7
            )
    ...
    @pytest.mark.asyncio
    async def test_chat_async_with_reasoning_model_parameter_adjustment(
        self: Any, client: Any
    ) -> None:
        ...
        with patch.object(
            client._async_client.chat.completions, "create", return_value=future
        ) as mock_create:
            messages = [ChatMessage(role=ChatRole.USER, content="Hello async")]
            result = await client.chat_async(
                messages=messages, model="o1-preview", max_tokens=20, temperature=0.9
            )
            assert isinstance(result, ChatCompletionResponse)
            mock_create.assert_called_once()
            call_args = mock_create.call_args[1]
...
class TestOpenAIClient:
    ...
    def test_init_missing_credentials(self: Any) -> None:
        ...
        with patch(
            "codestory.llm.client.get_settings", side_effect=Exception("No settings")
    ...
    def test_error_handling_complete(self: Any) -> None:
        ...
        with patch("openai.AzureOpenAI") as mock_azure_openai, patch(
            "openai.AsyncAzureOpenAI"
        ), patch("codestory.llm.backoff.retry_on_openai_errors"), patch(
            "codestory.llm.client.DefaultAzureCredential"
        ), patch(
            "codestory.llm.client.get_bearer_token_provider"
        ), patch(
