...
def test_llm_chat_message() -> None:
    ...
    # From dict
    message_dict = {"role": "system", "content": "You are a helpful assistant."}
...
def test_llm_embedding_request() -> None:
    ...
    request = EmbeddingRequest(
        model="text-embedding-3-small", input=["Hello world", "How are you?"]
    )
    assert request.model == "text-embedding-3-small"
...
def test_llm_chat_completion_response() -> None:
    ...
    message = ChatResponseMessage(role=ChatRole.ASSISTANT, content="Hello there!")
    choice = ChatResponseChoice(index=0, message=message, finish_reason="stop")
    usage = UsageInfo(prompt_tokens=15, completion_tokens=3, total_tokens=18)
...
def test_llm_embedding_response() -> None:
    ...
    usage = UsageInfo(prompt_tokens=8, total_tokens=8)

    response = EmbeddingResponse(
        object="list", data=[data1, data2], model="text-embedding-3-small", usage=usage
    )
