...
def mock_all_metrics_objects() -> None:
    ...
    with patch('prometheus_client.Counter') as mock_counter, patch('prometheus_client.Gauge') as mock_gauge, patch('prometheus_client.Histogram') as mock_histogram, patch('prometheus_client.registry.REGISTRY._names_to_collectors', {}):
...
class TestMetricsFunctions:
    ...
    def test_record_request(self: Any, mock_all_metrics_objects: Any) -> None:
        ...
        with patch.object(request_count, 'labels') as mock_count_labels, patch.object(request_duration, 'labels') as mock_duration_labels:
            mock_count_labels.return_value.inc = MagicMock()
            mock_duration_labels.return_value.observe = MagicMock()
            record_request(operation=OperationType.CHAT, model='gpt-4o', status='success', duration=1.5)
            mock_count_labels.assert_called_once_with(operation='chat', model='gpt-4o', status='success')
    ...
    def test_record_request_with_tokens(self: Any, mock_all_metrics_objects: Any) -> None:
        ...
        with patch.object(request_count, 'labels') as mock_count_labels, patch.object(request_duration, 'labels') as mock_duration_labels, patch.object(token_usage, 'labels') as mock_token_labels:
            mock_count_labels.return_value.inc = MagicMock()
            mock_duration_labels.return_value.observe = MagicMock()
            mock_token_labels.return_value.inc = MagicMock()
            record_request(operation=OperationType.CHAT, model='gpt-4o', status='success', duration=1.5, tokens={'prompt': 100, 'completion': 50, 'total': 150})
    ...
    def test_record_error(self: Any, mock_all_metrics_objects: Any) -> None:
        ...
        with patch.object(error_count, 'labels') as mock_error_labels:
            ...
            record_error(operation=OperationType.EMBEDDING, model='text-embedding-3-small', error_type='RateLimitError')
...
class TestInstrumentRequestDecorator:
    ...
    def test_instrument_request_success(self: Any, mock_all_metrics_objects: Any) -> None:
        ...
        mock_function.return_value = MagicMock(usage=MagicMock(prompt_tokens=100, completion_tokens=50, total_tokens=150))
        decorated_function = instrument_request(OperationType.CHAT)(mock_function)
        with patch.object(current_requests, 'labels') as mock_current, patch('codestory.llm.metrics.record_request') as mock_record:
    ...
    def test_instrument_request_error(self: Any, mock_all_metrics_objects: Any) -> None:
        ...
        with patch.object(current_requests, 'labels') as mock_current, patch('codestory.llm.metrics.record_request') as mock_record_req, patch('codestory.llm.metrics.record_error') as mock_record_err:
            mock_current.return_value.inc = MagicMock()
            mock_current.return_value.dec = MagicMock()
...
class TestInstrumentAsyncRequestDecorator:
    ...
    async def test_instrument_async_request_success(self: Any, mock_all_metrics_objects: Any) -> None:
        ...
        async def mock_coro(*args, **kwargs):
            return MagicMock(usage=MagicMock(prompt_tokens=100, completion_tokens=50, total_tokens=150))
