...
def mock_settings() -> Any:
    ...
    settings.model_dump.return_value = {'app_name': 'code-story', 'version': '0.1.0', 'log_level': 'INFO', 'auth_enabled': False, 'neo4j': {'uri': 'bolt://localhost:7687', 'username': 'neo4j', 'password': 'password', 'database': 'neo4j'}, 'redis': {'uri': 'redis://localhost:6379'}, 'openai': {'api_key': 'test-key', 'embedding_model': 'text-embedding-3-small', 'chat_model': 'gpt-4o', 'reasoning_model': 'gpt-4o', 'endpoint': 'https://api.openai.com/v1'}, 'azure_openai': {'deployment_id': 'gpt-4o', 'api_version': '2024-05-01'}, 'service': {'host': '0.0.0.0', 'port': 8000}, 'ingestion': {'config_path': 'pipeline_config.yml', 'chunk_size': 1024}, 'plugins': {'enabled': ['blarify', 'filesystem', 'summarizer', 'docgrapher']}, 'telemetry': {'metrics_port': 9090, 'log_format': 'json'}, 'interface': {'theme': 'dark', 'default_view': 'graph'}, 'azure': {'keyvault_name': 'test-keyvault', 'tenant_id': 'test-tenant'}}
    return settings

def test_redact_secrets() -> None:
    """Test redacting secrets from configuration dictionaries."""
    config = {'neo4j': {'uri': 'bolt://localhost:7687', 'username': 'neo4j', 'password': 'password'}, 'openai': {'api_key': 'test-key', 'embedding_model': 'text-embedding-3-small'}, 'azure': {'client_id': 'client-id', 'client_secret': 'client-secret'}}
...
def test_export_to_json(mock_settings: Any) -> None:
    ...
    mock_settings.model_dump.return_value = {'neo4j': {'uri': 'bolt://localhost:7687', 'username': 'neo4j', 'password': SecretStr('password'), 'database': 'neo4j'}, 'redis': {'uri': 'redis://localhost:6379'}, 'openai': {'api_key': SecretStr('test-key'), 'embedding_model': 'text-embedding-3-small', 'chat_model': 'gpt-4o', 'reasoning_model': 'gpt-4o', 'endpoint': 'https://api.openai.com/v1'}}
    with patch('src.codestory.config.export.get_settings', return_value=mock_settings), patch('json.dumps') as mock_json_dumps:
        ...
        try:
            with patch('src.codestory.config.export.settings_to_dict') as mock_settings_to_dict:
                mock_settings_to_dict.return_value = {'neo4j': {'uri': 'bolt://localhost:7687', 'username': 'neo4j', 'password': 'password'}, 'openai': {'api_key': 'test-key', 'embedding_model': 'text-embedding-3-small'}}
