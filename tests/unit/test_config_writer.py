...
def test_update_config_invalid_path() -> None:
    ...
    def mock_hasattr(obj: Any, name: Any):
        ...
        if obj == mock_settings:
            return name in {'neo4j', 'redis', 'openai', 'azure_openai', 'service', 'ingestion', 'plugins', 'telemetry', 'interface', 'azure'}
        if obj == mock_settings.neo4j:
            return name != 'invalid'
        return original_hasattr(obj, name)
    with patch('src.codestory.config.writer.get_settings', return_value=mock_settings), patch('builtins.hasattr', side_effect=mock_hasattr):
...
def test_update_config_persist_env() -> None:
    ...
    try:
        ...
        with patch('codestory.config.writer.get_settings', return_value=mock_settings), patch('codestory.config.writer.refresh_settings'), patch('codestory.config.writer.get_project_root') as mock_root, patch('codestory.config.writer.update_env') as mock_update_env:
...
def test_update_config_persist_toml() -> None:
    ...
    try:
        ...
        mock_settings.neo4j = MagicMock()
        mock_settings.neo4j.uri = 'bolt://localhost:7687'
        with patch('codestory.config.writer.get_settings', return_value=mock_settings), patch('codestory.config.writer.refresh_settings'), patch('codestory.config.writer.get_project_root') as mock_root, patch('codestory.config.writer.update_toml') as mock_update_toml:
