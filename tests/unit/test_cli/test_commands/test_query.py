...
class TestQueryCommands:
    ...
    def test_query_run_cypher(self, cli_runner: CliRunner, mock_service_client: MagicMock) -> None:
        ...
        with patch("codestory.cli.main.ServiceClient", return_value=mock_service_client):
            result = cli_runner.invoke(
                app, 
                ["query", "run", "MATCH (n) RETURN count(n) as count"]
            )
    ...
    def test_query_run_with_parameters(self, cli_runner: CliRunner, mock_service_client: MagicMock) -> None:
    ...
    def test_query_run_with_formats(self, cli_runner: CliRunner, mock_service_client: MagicMock) -> None:
        """Test 'query run' command with different output formats."""
        # Configure mock client
        mock_result = {
            "records": [
                {"name": "test_node", "type": "class", "path": "/path/to/test"}
            ]
        }
        mock_service_client.execute_query.return_value = mock_result
        ...
        for fmt in formats:
            ...
            with patch("codestory.cli.main.ServiceClient", return_value=mock_service_client):
                result = cli_runner.invoke(
                    app, 
                    ["query", "run", "MATCH (n) RETURN n.name, n.type, n.path", "--format", fmt]
                )
    ...
    def test_query_run_with_limit(self, cli_runner: CliRunner, mock_service_client: MagicMock) -> None:
        ...
        with patch("codestory.cli.main.ServiceClient", return_value=mock_service_client):
            result = cli_runner.invoke(
                app, 
                ["query", "run", "MATCH (n) RETURN n.id, n.name", "--limit", "3"]
            )
        ...
        with tempfile.NamedTemporaryFile(suffix=".json", delete=False) as temp_file:
            try:
                ...
                with patch("codestory.cli.main.ServiceClient", return_value=mock_service_client):
                    result = cli_runner.invoke(
                        app, 
                        ["query", "export", "MATCH (n) RETURN n.name, n.type", temp_file.name]
                    )
    ...
    def test_query_explore(self, cli_runner: CliRunner, mock_service_client: MagicMock) -> None:
        ...
        class_nodes = {
            "records": [
                {"n": {"name": "TestClass", "path": "/path/to/class"}}
            ]
        }
        
        function_nodes = {
            "records": [
                {"n": {"name": "testFunction", "path": "/path/to/function"}}
            ]
        }
        
        module_nodes = {
            "records": [
                {"n": {"name": "test_module", "path": "/path/to/module"}}
            ]
        }
