...
class TestAutoMount:
            ...
            with patch("subprocess.run") as mock_run:
                ...
                with patch(
                    ...
                    with patch.object(
                        ...
                        assert kwargs.get("dependencies") is None
    ...
    def test_no_auto_mount(self: Any, cli_runner: Any, cli_context: Any) -> None:
        ...
        with patch(
            ...
            cli_context["client"].base_url = "http://localhost:8000/v1"
            with patch("subprocess.run") as mock_run, \
                 patch("codestory.cli.commands.ingest.is_repo_mounted", return_value=True), \
    ...
    def test_non_docker_deployment(
        self: Any, cli_runner: Any, cli_context: Any
    ) -> None:
        ...
        with patch(
            ...
            with patch("subprocess.run") as mock_run, patch.object(
                ...
                mock_start.return_value = {"job_id": "test-job-id"}
                result = cli_runner.invoke(
                    ingest, ["start", repo_path, "--no-progress"], obj=cli_context
                )
                assert result.exit_code == 0
                assert mock_run.call_count == 0
                args, kwargs = mock_start.call_args
        ...
        with patch(
            ...
            with patch("subprocess.run"), \
                ...
                with patch(
                    ...
                    print("Exception:", result.exception)
                    print(
                        "mock_start.call_args:", getattr(mock_start, "call_args", None)
                    )
                    assert result.exit_code == 0
        ...
        with patch(
            ...
            with patch("subprocess.run"), \
                ...
                with patch(
                    ...
                    result = cli_runner.invoke(
                        ingest,
                        ["start", repo_path, "--no-auto-mount", "--no-progress"],
                        obj=cli_context,
                    )
                    print("CLI output:", result.output)
                    print("Exit code:", result.exit_code)
