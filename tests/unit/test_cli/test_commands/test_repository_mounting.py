...
class TestRepositoryMounting:
    ...
    def test_is_repo_mounted(self: Any) -> None:
        ...
        with patch('subprocess.run') as mock_run:
            mock_run.side_effect = [MagicMock(stdout='codestory-service', returncode=0), MagicMock(returncode=1)]
            result = ingest.is_repo_mounted('/test/repo', console=None)
            assert result is False
            mock_run.reset_mock()
            mock_run.side_effect = [MagicMock(stdout='codestory-service', returncode=0), MagicMock(returncode=0), MagicMock(stdout='exists', returncode=0)]
