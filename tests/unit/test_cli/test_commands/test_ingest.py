...
class TestIngestCommands:
    ...
    def test_ingest_start(self: Any, cli_runner: CliRunner, mock_service_client: MagicMock) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir, patch('codestory.cli.commands.ingest._show_progress'), patch('codestory.cli.main.ServiceClient', return_value=mock_service_client):
            with patch('codestory.cli.commands.ingest.setup_repository_mount', return_value=True), patch('codestory.cli.commands.ingest.wait_for_service', return_value=True):
    ...
    def test_ingest_start_with_priority(self: Any, cli_runner: CliRunner, mock_service_client: MagicMock) -> None:
        """Test 'ingest start' command with --priority option."""
        mock_service_client.start_ingestion.return_value = {'job_id': 'test-123'}
        mock_service_client.base_url = 'http://localhost:8000/v1'
        with tempfile.TemporaryDirectory() as temp_dir, patch('codestory.cli.commands.ingest._show_progress'), patch('codestory.cli.main.ServiceClient', return_value=mock_service_client):
            with patch('codestory.cli.commands.ingest.setup_repository_mount', return_value=True), patch('codestory.cli.commands.ingest.wait_for_service', return_value=True):
                result = cli_runner.invoke(app, ['ingest', 'start', temp_dir, '--priority', 'high'])
            assert result.exit_code == 0
            assert 'Starting ingestion' in result.output
            assert 'test-123' in result.output
            mock_service_client.start_ingestion.assert_called_once()
    ...
    def test_ingest_start_no_progress(self: Any, cli_runner: CliRunner, mock_service_client: MagicMock) -> None:
        """Test 'ingest start --no-progress' command."""
        mock_service_client.start_ingestion.return_value = {'job_id': 'test-123'}
        mock_service_client.base_url = 'http://localhost:8000/v1'
        with tempfile.TemporaryDirectory() as temp_dir, patch('codestory.cli.commands.ingest._show_progress') as mock_show_progress:
            with patch('codestory.cli.main.ServiceClient', return_value=mock_service_client), patch('codestory.cli.commands.ingest.setup_repository_mount', return_value=True), patch('codestory.cli.commands.ingest.wait_for_service', return_value=True):
                result = cli_runner.invoke(app, ['ingest', 'start', temp_dir, '--no-progress'])
            assert result.exit_code == 0
            assert 'Starting ingestion' in result.output
            assert 'test-123' in result.output
            mock_show_progress.assert_not_called()

    def test_ingest_start_error(self: Any, cli_runner: CliRunner, mock_service_client: MagicMock) -> None:
    ...
    def test_ingest_status(self: Any, cli_runner: CliRunner, mock_service_client: MagicMock, sample_ingestion_status: dict) -> None:
    ...
    def test_ingest_stop(self: Any, cli_runner: CliRunner, mock_service_client: MagicMock) -> None:
    ...
    def test_ingest_stop_error(self: Any, cli_runner: CliRunner, mock_service_client: MagicMock) -> None:
        """Test 'ingest stop' with error."""
        mock_service_client.stop_ingestion.return_value = {'success': False, 'message': 'Job not found'}
        with patch('codestory.cli.main.ServiceClient', return_value=mock_service_client):
            result = cli_runner.invoke(app, ['ingest', 'stop', 'test-123'])
        assert result.exit_code == 0
        assert 'Error' in result.output
    ...
    def test_ingest_jobs(self: Any, cli_runner: CliRunner, mock_service_client: MagicMock) -> None:
        """Test 'ingest jobs' command."""
        mock_service_client.list_ingestion_jobs.return_value = [{'job_id': 'job-1', 'status': 'completed', 'repository_path': '/path/1', 'created_at': '2023-01-01', 'progress': 100}, {'job_id': 'job-2', 'status': 'running', 'repository_path': '/path/2', 'created_at': '2023-01-02', 'progress': 50}]
        with patch('codestory.cli.main.ServiceClient', return_value=mock_service_client):
    ...
    def test_ingest_jobs_empty(self: Any, cli_runner: CliRunner, mock_service_client: MagicMock) -> None:
