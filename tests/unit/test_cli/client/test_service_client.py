...
class TestServiceClient:
    ...
    def test_generate_visualization_logging(self: Any) -> None:
        ...
        client = ServiceClient(console=console)
        with patch('httpx.Client.get') as mock_get:
            mock_get.side_effect = [httpx.HTTPError('First endpoint failed'), MagicMock(text='<html>Test Visualization</html>')]
    ...
    def test_list_ingestion_jobs_with_items_field(self: Any) -> None:
        ...
        with patch('httpx.Client.get') as mock_get:
            mock_response = MagicMock()
            mock_response.raise_for_status.return_value = None
            mock_response.json.return_value = {'items': [{'job_id': 'job1', 'status': 'running'}, {'job_id': 'job2', 'status': 'completed'}], 'total': 2, 'limit': 10, 'offset': 0, 'has_more': False}
    ...
    def test_list_ingestion_jobs_with_jobs_field(self: Any) -> None:
        ...
        with patch('httpx.Client.get') as mock_get:
            mock_response = MagicMock()
            mock_response.raise_for_status.return_value = None
            mock_response.json.return_value = {'jobs': [{'job_id': 'job1', 'status': 'running'}, {'job_id': 'job2', 'status': 'completed'}]}
            mock_get.return_value = mock_response
            jobs = client.list_ingestion_jobs()
            assert len(jobs) == 2
            assert jobs[0]['job_id'] == 'job1'
    ...
    def test_list_ingestion_jobs_with_list_format(self: Any) -> None:
        ...
        with patch('httpx.Client.get') as mock_get:
            mock_response = MagicMock()
            mock_response.raise_for_status.return_value = None
            mock_response.json.return_value = [{'job_id': 'job1', 'status': 'running'}, {'job_id': 'job2', 'status': 'completed'}]
            mock_get.return_value = mock_response
            jobs = client.list_ingestion_jobs()
            assert len(jobs) == 2
            assert jobs[0]['job_id'] == 'job1'
    ...
    def test_list_ingestion_jobs_with_single_job_format(self: Any) -> None:
        ...
        with patch('httpx.Client.get') as mock_get:
            mock_response = MagicMock()
            mock_response.raise_for_status.return_value = None
            mock_response.json.return_value = {'job_id': 'job1', 'status': 'running', 'progress': 50.0}
            mock_get.return_value = mock_response
            jobs = client.list_ingestion_jobs()
            assert len(jobs) == 1
            assert jobs[0]['job_id'] == 'job1'
    ...
    def test_list_ingestion_jobs_with_mock_format(self: Any) -> None:
        ...
        with patch('httpx.Client.get') as mock_get:
            mock_response = MagicMock()
            mock_response.raise_for_status.return_value = None
            mock_response.json.return_value = {'job_id': 'jobs', 'status': 'pending', 'progress': 0.0}
            mock_get.return_value = mock_response
            jobs = client.list_ingestion_jobs()
            assert len(jobs) == 1
