        ...
        try:
            ...
            if re.search(pattern1, content):
                ...
                if 'neo4j_port = ' not in content:
                    content = re.sub('import os', 'import os\n\n# Determine Neo4j port based on CI environment\nci_env = os.environ.get("CI") == "true"\nneo4j_port = "7687" if ci_env else "7688"', content)
...
def pytest_addoption(parser: Any) -> None:
    ...
    parser.addoption('--skip-neo4j', action='store_true', default=False, help='Skip tests that require Neo4j')
    parser.addoption('--skip-celery', action='store_true', default=False, help='Skip tests that require Celery')
    parser.addoption('--run-neo4j', action='store_true', default=False, help='[DEPRECATED] Use --skip-neo4j=False instead')
    parser.addoption('--run-celery', action='store_true', default=False, help='[DEPRECATED] Use --skip-celery=False instead')
...
def celery_app(redis_client: Any) -> None:
    ...
    task_modules = ['codestory.ingestion_pipeline.tasks', 'codestory_filesystem.step', 'codestory_blarify.step', 'codestory_summarizer.step', 'codestory_docgrapher.step']
