...
class TestFilesystemIngestionSimple:
    ...
    def create_test_repository(self: Any) -> None:
        ...
        (self.test_repo_path / "src/main/app.py").write_text(
            '"""Main application."""\n\ndef main():\n    print("Hello, World!")\n\nif __name__ == "__main__":\n    main()\n'
        )
        (self.test_repo_path / "src/test/test_app.py").write_text(
            '"""Tests for app."""\n\ndef test_main():\n    assert True\n'
        )
        (self.test_repo_path / "docs/guide.md").write_text(
        ...
        (self.test_repo_path / "src/main/__pycache__/app.pyc").write_bytes(
            b"\x00\x01\x02\x03"
        )
        (self.test_repo_path / "build").mkdir(exist_ok=True)
        (self.test_repo_path / "build/output.js").write_text("compiled output")
        logger.info(
            f"Test repository created with {len(list(self.test_repo_path.rglob('*')))} items"
        )
    ...
    def test_filesystem_step_with_various_file_types(self: Any) -> None:
        ...
        (self.test_repo_path / "markdown_file.md").write_text(
            "# Markdown\n\nContent here"
        )
        (self.test_repo_path / "json_file.json").write_text('{"key": "value"}')
        (self.test_repo_path / "yaml_file.yaml").write_text(
            "key: value\nlist:\n  - item1\n  - item2"
        )
        (self.test_repo_path / "python_file.py").write_text("# Python\nprint('hello')")
        (self.test_repo_path / "javascript_file.js").write_text(
            "// JavaScript\nconsole.log('hello');"
        )
