...
class TestRepositoryMounting:
    ...
    def test_mount_verification(self: Any, temp_repository: Any) -> None:
        ...
        start_result = subprocess.run(['docker', 'run', '-d', '--name', container_name, 'alpine', 'sleep', '60'], capture_output=True, text=True, check=False)
        print(f'Container start result: {start_result.stdout}')
        container_check = subprocess.run(['docker', 'ps', '--filter', f'name={container_name}', '--format', '{{.Names}}'], capture_output=True, text=True)
        ...
        path_check = subprocess.run(f"docker exec {container_name} sh -c 'test -d {container_path} && echo exists'", shell=True, capture_output=True, text=True, check=False)
        assert 'exists' not in path_check.stdout, 'Repository path should not be accessible without mounting'
        print('Test 1 passed: Path not accessible without mounting')
        subprocess.run(['docker', 'rm', '-f', container_name], capture_output=True)
        print('Test 2: Starting container with repository mounted')
        mount_result = subprocess.run(['docker', 'run', '-d', '--name', container_name, '-v', f'{temp_repository}:{container_path}', 'alpine', 'sleep', '60'], capture_output=True, text=True, check=False)
        ...
        path_check = subprocess.run(f"docker exec {container_name} sh -c 'test -d {container_path} && echo exists'", shell=True, capture_output=True, text=True, check=False)
        assert 'exists' in path_check.stdout, f'Repository path not accessible in container even with mounting: {container_path}'
        print('Test 2 passed: Path accessible with mounting')
        ls_result = subprocess.run(['docker', 'exec', container_name, 'ls', '-la', container_path], capture_output=True, text=True)
        ...
        assert '.mount_test' in ls_result.stdout, f'Repository not mounted correctly: {ls_result.stdout}'
        cat_result = subprocess.run(['docker', 'exec', container_name, 'cat', f'{container_path}/.mount_test'], capture_output=True, text=True)
        assert 'Mount test marker created at' in cat_result.stdout, f'Could not read mount test file: {cat_result.stdout}'
        inspect_result = subprocess.run(['docker', 'inspect', container_name, '--format', '{{json .Mounts}}'], capture_output=True, text=True)
        assert temp_repository in inspect_result.stdout, f'Repository not found in Docker mount configuration: {inspect_result.stdout}'
        subprocess.run(['docker', 'rm', '-f', container_name], capture_output=True)
...
class TestCliAutoMount:
    ...
    def test_cli_mount_verification_function(self: Any, temp_repository: Any) -> None:
        ...
        cli_context = {'client': mock_client, 'console': console, 'settings': MagicMock()}
        container_name = 'codestory-service'
        subprocess.run(['docker', 'rm', '-f', container_name], capture_output=True)
        subprocess.run(['docker', 'run', '-d', '--name', container_name, 'alpine', 'sleep', '60'], capture_output=True)
        container_check = subprocess.run(['docker', 'ps', '--filter', f'name={container_name}', '--format', '{{.Names}}'], capture_output=True, text=True)
        assert container_name in container_check.stdout, 'Test container is not running'
        ...
        try:
            with pytest.raises(Exception) as excinfo:
                ...
                start_ingestion(click_ctx, temp_repository, no_progress=True, container=True, path_prefix='/repositories', auto_mount=False, no_auto_mount=True)
            print(f'Got expected exception: {excinfo.value!s}')
            output = console.file.getvalue()
            assert 'Repository not mounted' in output or 'does not exist' in str(excinfo.value), f'CLI did not detect missing mount. Output: {output}, Exception: {excinfo.value}'
            print('Test 1 passed: CLI correctly detected unmounted repository')
        finally:
            subprocess.run(['docker', 'rm', '-f', container_name], capture_output=True)
        subprocess.run(['docker', 'run', '-d', '--name', container_name, '-v', f'{temp_repository}:{container_path}', 'alpine', 'sleep', '60'], capture_output=True)
        try:
            console = Console(file=StringIO())
