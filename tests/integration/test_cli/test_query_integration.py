...
class TestQueryCommands:
    ...
    def test_query_run_cypher(self, cli_runner: CliRunner, running_service: dict[str, Any]) -> None:
        ...
        result = cli_runner.invoke(
            app, 
            ["query", "run", "MATCH (n) RETURN count(n) as count LIMIT 5"]
        )
    ...
    def test_query_run_with_format(self, cli_runner: CliRunner, running_service: dict[str, Any]) -> None:
        ...
        result = cli_runner.invoke(
            app, 
            ["query", "run", "MATCH (n) RETURN count(n) as count LIMIT 5", "--format", "json"]
        )
        ...
        result = cli_runner.invoke(
            app, 
            ["query", "run", "MATCH (n) RETURN count(n) as count LIMIT 5", "--format", "csv"]
        )
    ...
    def test_query_run_with_limit(self, cli_runner: CliRunner, running_service: dict[str, Any]) -> None:
        ...
        result = cli_runner.invoke(
            app, 
            ["query", "run", "MATCH (n) RETURN n LIMIT 100", "--limit", "3"]
        )
    ...
    def test_query_explore(self, cli_runner: CliRunner, running_service: dict[str, Any]) -> None:
        ...
        assert "Node Types in Graph" in result.output or \
               "Relationship Types in Graph" in result.output or \
               "Example queries" in result.output