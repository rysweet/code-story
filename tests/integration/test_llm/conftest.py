...
def pytest_addoption(parser: Any) -> None:
    ...
    parser.addoption('--skip-openai', action='store_true', default=False, help='Skip tests that require OpenAI API access')
    parser.addoption('--run-openai', action='store_true', default=False, help='[DEPRECATED] Use --skip-openai=False instead')
...
def openai_credentials() -> dict[str, Any]:
    ...
    try:
        ...
        return {'endpoint': endpoint, 'tenant_id': getattr(settings.openai, 'tenant_id', None), 'subscription_id': getattr(settings.openai, 'subscription_id', None), 'embedding_model': settings.openai.embedding_model, 'chat_model': settings.openai.chat_model, 'reasoning_model': settings.openai.reasoning_model}
    except Exception:
        pass
    ...
    return {'endpoint': endpoint, 'tenant_id': os.environ.get('OPENAI__TENANT_ID') or os.environ.get('AZURE_TENANT_ID'), 'subscription_id': sub_id, 'embedding_model': os.environ.get('OPENAI__EMBEDDING_MODEL', 'text-embedding-3-small'), 'chat_model': os.environ.get('OPENAI__CHAT_MODEL', 'gpt-4o'), 'reasoning_model': os.environ.get('OPENAI__REASONING_MODEL', 'gpt-4o')}
...
def azure_login(openai_credentials: Any) -> None:
    ...
    if not tenant_id:
        return
    try:
        tenant_result = subprocess.run(['az', 'account', 'show', '--query', 'tenantId', '-o', 'tsv'], check=False, capture_output=True, text=True)
        ...
        if tenant_result.returncode != 0 or current_tenant != tenant_id:
            pytest.skip(f'Azure CLI not logged into the correct tenant. Run:\naz login --tenant {tenant_id}')
        if subscription_id:
            subprocess.run(['az', 'account', 'set', '--subscription', subscription_id], check=False, capture_output=True)
    except Exception:
