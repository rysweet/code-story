...
def test_chat_completion(client: Any) -> None:
    ...
    """
    messages = [ChatMessage(role=ChatRole.SYSTEM, content='You are a helpful assistant that answers concisely.'), ChatMessage(role=ChatRole.USER, content='What is 2+2?')]
...
def test_regular_model_vs_reasoning_model_parameters(client: Any) -> None:
    ...
    reasoning_result = client.chat(messages=messages, model=reasoning_model, max_tokens=5)
...
async def test_chat_async(client) -> None:
    ...
    """
    messages = [ChatMessage(role=ChatRole.SYSTEM, content='You are a helpful assistant that answers concisely.'), ChatMessage(role=ChatRole.USER, content='What is the capital of France?')]
