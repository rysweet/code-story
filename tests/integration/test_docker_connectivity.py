...
class TestDockerConnectivityFix:
    ...
    def worker_container(self: Any) -> None:
        ...
        result = subprocess.run(['docker', 'compose', 'up', '-d', 'redis'], capture_output=True, text=True, cwd=Path.cwd())
        if result.returncode != 0:
            pytest.skip('Failed to start Redis for Docker test')
        result = subprocess.run(['docker', 'compose', 'up', '-d', 'worker'], capture_output=True, text=True, cwd=Path.cwd())
        if result.returncode != 0:
            pytest.skip('Failed to start worker for Docker test')
        time.sleep(45)
        result = subprocess.run(['docker', 'ps', '--filter', 'name=codestory-worker', '--format', '{{.Names}}'], capture_output=True, text=True)
    ...
    def test_docker_socket_accessibility(self: Any, worker_container: Any) -> None:
        ...
        result = subprocess.run(['docker', 'exec', worker_container, 'ls', '-la', '/var/run/docker.sock'], capture_output=True, text=True)
    ...
    def test_docker_cli_installation(self: Any, worker_container: Any) -> None:
        ...
        result = subprocess.run(['docker', 'exec', worker_container, 'which', 'docker'], capture_output=True, text=True)
    ...
    def test_docker_python_module_availability(self: Any, worker_container: Any) -> None:
        """Test that Python docker module is available in the virtual environment"""
        result = subprocess.run(['docker', 'exec', worker_container, '/app/.venv/bin/python', '-c', "import docker; print('Docker module imported successfully')"], capture_output=True, text=True)
    ...
    def test_docker_daemon_connectivity(self: Any, worker_container: Any) -> None:
        ...
        result = subprocess.run(['docker', 'exec', worker_container, '/app/.venv/bin/python', '-c', "import docker; client = docker.from_env(); print('Docker client created'); print('SUCCESS')"], capture_output=True, text=True, timeout=30)
    ...
    def test_docker_group_configuration(self: Any, worker_container: Any) -> None:
        ...
        result = subprocess.run(['docker', 'exec', worker_container, 'getent', 'group', 'docker'], capture_output=True, text=True)
        if result.returncode == 0:
    ...
    def test_blarify_docker_integration(self: Any, worker_container: Any) -> None:
        ...
        result = subprocess.run(['docker', 'exec', worker_container, '/app/.venv/bin/python', '-c', test_code], capture_output=True, text=True, timeout=45)
    ...
    def test_worker_health_with_docker_fix(self: Any, worker_container: Any) -> None:
        ...
        result = subprocess.run(['docker', 'exec', worker_container, 'ps', 'aux'], capture_output=True, text=True)
