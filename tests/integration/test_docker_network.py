...
def test_service_to_neo4j_connectivity(docker_compose_project):
    ...
    assert "200" in result["stdout"] or "302" in result["stdout"], f"Expected 200 or 302 status code, got: {result['stdout']}"
...
def test_service_to_redis_connectivity(docker_compose_project):
    ...
    result = exec_in_container(
        service_container,
        ["timeout", "5", "redis-cli", "-h", "redis", "ping"]
    )
...
def test_external_health_endpoint(docker_compose_project):
    ...
    for endpoint in ["/health", "/v1/health"]:
        ...
        assert result.returncode == 0, f"Failed to connect to external health endpoint: {result.stderr}"
        
        try:
            health_data = json.loads(result.stdout)
            assert "status" in health_data, f"Health response missing status field in endpoint {endpoint}"
        except json.JSONDecodeError:
