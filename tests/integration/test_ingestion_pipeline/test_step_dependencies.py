...
def mock_steps() -> Generator[dict[str, Any], None, None]:
    ...
    execution_order = []
    with patch.object(
        FileSystemStep, "run", autospec=True
    ) as mock_fs_run, patch.object(
        BlarifyStep, "run", autospec=True
    ) as mock_blarify_run, patch.object(
        SummarizerStep, "run", autospec=True
    ) as mock_summarizer_run, patch.object(
        ...
        with patch.object(
            FileSystemStep, "status", autospec=True
        ) as mock_fs_status, patch.object(
            BlarifyStep, "status", autospec=True
        ) as mock_blarify_status, patch.object(
            SummarizerStep, "status", autospec=True
        ) as mock_summarizer_status, patch.object(
...
def test_parallel_execution_where_possible(
    ...
    with patch("codestory.ingestion_pipeline.tasks.group") as mock_group, patch(
        "codestory.ingestion_pipeline.tasks.chain"
    ) as mock_chain:
...
def test_error_handling_in_dependency_chain() -> None:
    ...
    def check_dependencies(step_name: Any, repo_path: Any):
        dependencies = []
        if (
            "dependencies" in manager.config
            and step_name in manager.config["dependencies"]
        ):
...
async def test_job_dependency_orchestration(sample_repo: Any) -> None:
    ...
    if service.redis:
        ...
        assert (
            not waiting
        ), "Job B should have been dequeued and started after dependency completion"
