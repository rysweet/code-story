...
def initialized_repo(sample_repo, neo4j_connector):
    ...
    ), f"FileSystemStep failed: {status.get('error')}"
    ...
    sample_file = neo4j_connector.run_query(
        "MATCH (f:File {path: 'src/sample.py'}) RETURN ID(f) as id", fetch_one=True
    )
    ...
    if sample_file:
        ...
        class_result = neo4j_connector.run_query(
            class_query, parameters={"file_id": sample_file_id}, fetch_one=True
        )
        ...
        neo4j_connector.run_query(main_query, parameters={"file_id": sample_file_id})
...
def test_docgrapher_step_run(initialized_repo, neo4j_connector, mock_llm_client):
    ...
    doc_count = neo4j_connector.run_query(
        "MATCH (d:Documentation) RETURN COUNT(d) as count", fetch_one=True
    )["count"]
    ...
    entity_count = neo4j_connector.run_query(
        "MATCH (e:DocumentationEntity) RETURN COUNT(e) as count", fetch_one=True
    )["count"]
    ...
    readme_doc = neo4j_connector.run_query(
        "MATCH (d:Documentation {name: 'README.md'}) RETURN d", fetch_one=True
    )
    assert readme_doc is not None, "README.md documentation not found"

    api_doc = neo4j_connector.run_query(
        "MATCH (d:Documentation {name: 'api.md'}) RETURN d", fetch_one=True
    )
