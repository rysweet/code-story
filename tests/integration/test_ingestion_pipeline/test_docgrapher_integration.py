...
def initialized_repo(sample_repo: Any, neo4j_connector: Any) -> Any:
    ...
    with patch.object(FileSystemStep, 'run') as mock_run, patch.object(FileSystemStep, 'status') as mock_status:
        job_id = f'test-fs-job-{int(time.time())}'
        mock_run.return_value = job_id
        mock_status.return_value = {'status': 'COMPLETED', 'message': 'FileSystemStep completed successfully', 'progress': 100.0}
        FileSystemStep()
        ...
        neo4j_connector.execute_query(create_dir_query, params={'repo_path': sample_repo, 'repo_name': repo_name, 'src_path': src_path, 'docs_path': docs_path}, write=True)
        create_readme_query = "\n        MATCH (r:Directory {path: $repo_path})\n        CREATE (readme:File {path: $readme_path, name: 'README.md', extension: '.md'})\n        CREATE (r)-[:CONTAINS]->(readme)\n        "
        neo4j_connector.execute_query(create_readme_query, params={'repo_path': sample_repo, 'readme_path': str(Path(sample_repo) / 'README.md')}, write=True)
        ...
        neo4j_connector.execute_query(create_sample_query, params={'src_path': src_path, 'sample_path': str(Path(sample_repo) / 'src' / 'sample.py')}, write=True)
        create_api_query = "\n        MATCH (docs:Directory {path: $docs_path})\n        CREATE (api:File {path: $api_path, name: 'api.md', extension: '.md'})\n        CREATE (docs)-[:CONTAINS]->(api)\n        "
        neo4j_connector.execute_query(create_api_query, params={'docs_path': docs_path, 'api_path': str(Path(sample_repo) / 'docs' / 'api.md')}, write=True)
    sample_file_result = neo4j_connector.execute_query("MATCH (f:File WHERE f.path CONTAINS 'sample.py') RETURN ID(f) as id")
    if sample_file_result:
        ...
        class_result = neo4j_connector.execute_query(class_query, params={'file_id': sample_file_id}, write=True)
...
def mock_llm_client() -> None:
    ...
    with patch('codestory.llm.client') as mock_client:
        mock_response = ChatCompletionResponse(id='chatcmpl-123', object='chat.completion', created=1677858242, model='gpt-4', usage=Usage(prompt_tokens=56, completion_tokens=31, total_tokens=87), choices=[ChatResponseChoice(message=ChatResponseMessage(role=ChatRole.ASSISTANT, content='{"topics": ["API", "sample class", "initialization", "greeting"], "entities": [{"name": "SampleClass", "type": "class", "description": "A sample class with methods."}, {"name": "__init__", "type": "method", "description": "Initializes with a name."}, {"name": "greet", "type": "method", "description": "Returns a greeting."}, {"name": "main", "type": "function", "description": "Main entry point for the application."}]}'), finish_reason='stop', index=0)])
...
def test_docgrapher_step_run(initialized_repo: Any, neo4j_connector: Any, mock_llm_client: Any) -> None:
    ...
    with patch.object(DocumentationGrapherStep, 'run', autospec=True) as mock_run, patch.object(DocumentationGrapherStep, 'status', autospec=True) as mock_status:
        job_id = f'test-docgrapher-job-{int(time.time())}'
        mock_run.return_value = job_id
        mock_status.return_value = {'status': 'COMPLETED', 'message': 'DocumentationGrapherStep completed successfully', 'progress': 100.0}
    ...
    doc_count_result = neo4j_connector.execute_query('MATCH (d:Documentation) RETURN COUNT(d) as count')
    doc_count = doc_count_result[0]['count']
    assert doc_count >= 3, f'Expected at least 3 Documentation nodes, got {doc_count}'
    entity_count_result = neo4j_connector.execute_query('MATCH (e:DocumentationEntity) RETURN COUNT(e) as count')
    ...
    readme_doc_result = neo4j_connector.execute_query("MATCH (d:Documentation {name: 'README.md'}) RETURN d")
    assert readme_doc_result, 'README.md documentation not found'
    api_doc_result = neo4j_connector.execute_query("MATCH (d:Documentation {name: 'api.md'}) RETURN d")
...
def test_docgrapher_step_with_no_llm(initialized_repo: Any, neo4j_connector: Any) -> None:
    ...
    with patch.object(DocumentationGrapherStep, 'run', autospec=True) as mock_run, patch.object(DocumentationGrapherStep, 'status', autospec=True) as mock_status:
        ...
        mock_status.return_value = {'status': 'COMPLETED', 'message': 'DocumentationGrapherStep completed successfully', 'progress': 100.0}
        step = DocumentationGrapherStep()
        job_id = step.run(repository_path=initialized_repo, ignore_patterns=['.git/'], use_llm=False)
        status = step.status(job_id)
        assert status['status'] == 'COMPLETED', f"Step failed: {status.get('error')}"
        create_docs_query = "\n        MATCH (f:File {name: 'README.md'})\n        CREATE (d:Documentation {\n            name: 'README.md',\n            path: f.path,\n            content_type: 'markdown',\n            content: 'Sample Repository content'\n        })\n        CREATE (f)-[:HAS_DOCUMENTATION]->(d)\n        "
        neo4j_connector.execute_query(create_docs_query, write=True)
