...
def test_retry_and_failure_reporting(tmp_path: Any) -> None:
    ...
    with patch.object(FileSystemStep, "run", flaky_run), patch.object(
        ...
        manager.config["steps"] = [
            {"name": "filesystem", "max_retries": 2, "back_off_seconds": 1}
        ]
...
def test_retry_exceeds_max(tmp_path: Any) -> None:
    ...
    with patch.object(FileSystemStep, "run", always_fail_run), patch.object(
        ...
        assert (
            fs_step["status"] == StepStatus.FAILED
        ), "Step should fail after max retries"
        assert (
            fs_step["last_error"] == "Simulated persistent error"
        ), "Last error should be reported"
