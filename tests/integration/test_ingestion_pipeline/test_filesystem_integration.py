...
def test_filesystem_step_ingestion_update(sample_repo, neo4j_connector, celery_app):
    ...
    # Print configuration for debugging
    print(f"Running initial indexing...")
    ...
    print(f"Adding new file to repository...")
    ...
    assert update_status["status"] == StepStatus.COMPLETED, f"Update step failed: {update_status.get('error')}"
