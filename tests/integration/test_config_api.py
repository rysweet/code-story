...
def test_client(neo4j_connector: Any) -> None:
    ...
    test_user = {'sub': 'test-user-id', 'name': 'Test User', 'email': 'test@example.com', 'roles': ['admin'], 'exp': int(time.time()) + 3600}
    ...
    with mock.patch('codestory_service.infrastructure.openai_adapter.OpenAIAdapter.check_health') as mock_openai:
        mock_openai.return_value = {'status': 'healthy', 'details': {'models': {'embedding': 'text-embedding-ada-002', 'chat': 'gpt-4', 'reasoning': 'gpt-4'}}}
        os.environ['NEO4J_DATABASE'] = 'testdb'
