...
class OpenAIServiceAdapter:
    ...
    async def generate_code_summary(
        self: Any, code: str, context: str | None = None, max_tokens: int = 500
    ) -> str:
        ...
        try:
            messages = [
                ChatMessage(
                    ...
                    content="You are a helpful assistant that summarizes code. Provide a clear, concise explanation of what the code does, focusing on its purpose, functionality, and key components. Be specific and technical, but avoid unnecessary details.",
                )
            ]
            if context:
                messages.append(
                    ChatMessage(
                        role=ChatRole.USER,
                        content=f"I need to understand this code in the context of: {context}\n\nPlease summarize what it does, its main purpose, and how it works.",
                    )
                )
            else:
                messages.append(
                    ChatMessage(
                        role=ChatRole.USER,
                        content="Please summarize what this code does, its main purpose, and how it works.",
                    )
                )
            messages.append(
                ChatMessage(role=ChatRole.USER, content=f"```\n{code}\n```")
            )
    ...
    async def generate_path_explanation(
        self: Any, path_elements: list[dict[str, Any]], max_tokens: int = 300
    ) -> str:
        ...
        try:
            ...
            messages = [
                ChatMessage(
                    role=ChatRole.SYSTEM,
                    content="You are a helpful assistant that explains relationships between code elements. Based on the path between code elements, explain how they are related and what this relationship means for the codebase.",
                ),
                ChatMessage(
                    role=ChatRole.USER,
                    content=f"Please explain the following path between code elements in a clear, concise way. Focus on how these elements interact and what this relationship reveals about the code structure and behavior.\n\n{path_description}",
                ),
            ]
            request = ChatCompletionRequest(
                messages=messages,
            ...
            if explanation is None:
                raise ToolError(
                    "OpenAI returned no explanation.",
                    status_code=status.HTTP_502_BAD_GATEWAY,
                )
            duration = time.time() - start_time
