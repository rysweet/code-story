...
class SearchGraphTool(BaseTool):
    ...
    parameters = {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'Search term or natural language description'}, 'node_types': {'type': 'array', 'items': {'type': 'string'}, 'description': "Optional filter for specific node types (e.g., 'Class', 'Function')"}, 'limit': {'type': 'integer', 'description': 'Maximum number of results to return'}}, 'required': ['query']}
    ...
    async def __call__(self: Any, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if isinstance(e, ToolError):
                raise
            raise ToolError(f'Search failed: {e!s}', status_code=status.HTTP_500_INTERNAL_SERVER_ERROR) from e