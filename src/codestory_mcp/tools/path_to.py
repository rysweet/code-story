...
class PathToTool(BaseTool):
    ...
    parameters = {'type': 'object', 'properties': {'from_id': {'type': 'string', 'description': 'ID of the source node'}, 'to_id': {'type': 'string', 'description': 'ID of the target node'}, 'max_paths': {'type': 'integer', 'description': 'Maximum number of paths to return'}, 'include_explanation': {'type': 'boolean', 'description': 'Whether to include a natural language explanation of the path'}}, 'required': ['from_id', 'to_id']}
    ...
    async def __call__(self: Any, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if not from_id:
            raise ToolError('Source node ID cannot be empty', status_code=status.HTTP_400_BAD_REQUEST)
        if not to_id:
            raise ToolError('Target node ID cannot be empty', status_code=status.HTTP_400_BAD_REQUEST)
        if max_paths < 1:
            raise ToolError('Maximum paths must be a positive integer', status_code=status.HTTP_400_BAD_REQUEST)
        logger.info('Finding paths between nodes', from_id=from_id, to_id=to_id, max_paths=max_paths)
        try:
            paths = await self.graph_service.find_paths(from_id=from_id, to_id=to_id, max_paths=max_paths)
            response = RelationshipSerializer.to_mcp_path_result(paths=paths, exclude_node_properties=['content'])
            if include_explanation and paths:
                first_path = paths[0]
                path_elements: list[Any] = []
                for i, element in enumerate(first_path):
                    if i % 2 == 0:
                        path_elements.append({'element_type': 'node', 'id': str(element.id), 'type': element.labels[0] if element.labels else 'Unknown', 'name': element.get('name', ''), 'content': element.get('content', '')[:200]})
                    else:
            ...
            if isinstance(e, ToolError):
                raise
            raise ToolError(f'Path finding failed: {e!s}', status_code=status.HTTP_500_INTERNAL_SERVER_ERROR) from e