...
class FileSystemStep(PipelineStep):
    ...
    def run(self: Any, repository_path: str, **config: Any) -> str:
        ...
        try:
            ...
            log_debug(
                f"Submitting task with args=[{repository_path}], kwargs={kwargs}",
                job_id,
            )
    ...
    def status(self: Any, job_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            log_debug(
                f"Celery AsyncResult status: {result.status}, ready: {result.ready()}",
                job_id,
            )
            ...
            if result.ready():
                ...
                if result.successful():
                    ...
                    log_error(error_msg, job_id=job_id)
                    status_info.update(
                        {"status": StepStatus.FAILED, "error": str(result.result)}
                    )
            else:
            ...
            log_error(error_msg, error=e, job_id=job_id)
            job_info.update(
                {"status": StepStatus.FAILED, "error": f"Status check failed: {e!s}"}
            )
            return job_info  # type: ignore[no-any-return]
...
def process_filesystem(
    ...
    **config: Any,
    ...
    log_debug(
        f"Neo4j settings from config: uri={settings.neo4j.uri}, database={settings.neo4j.database}",
        job_id,
    )
    ...
    if not neo4j:
        ...
        self.update_state(
            state="FAILURE",
            meta={"error": error_msg, "job_id": job_id, "details": error_details},
        )
        return {
            "status": StepStatus.FAILED,
    ...
    try:
        ...
        dir_count = 0
        log_info(
            f"Starting repository traversal with max_depth: {max_depth} (unlimited if None)",
            job_id,
        )
        ...
        try:
            ...
            if not repo_node:
                log_error(
                    "Failed to create repository node - query returned no results",
                    job_id=job_id,
                )
        except Exception as e:
        ...
        for current_dir, dirs, files in os.walk(repository_path):
            ...
            rel_path_posix = (
                pathlib.Path(rel_path).as_posix() if rel_path != "." else ""
            )
            dirs_to_remove = []
            for d in list(dirs):
            ...
            if rel_dir_path == ".":
                ...
                try:
                    ...
                    if parent_path == "":
                        ...
                        log_debug(
                            f"Linking directory {rel_dir_path} to parent directory {parent_path}",
                            job_id,
                        )
                    ...
                    try:
                        ...
                        log_error(
                            "Error updating directory progress state",
                            error=e,
                            job_id=job_id,
                        )
                except Exception as e:
            ...
            for file_idx, file in enumerate(files):
                ...
                try:
                    ...
                    if rel_dir_path == ".":
                        ...
                        log_debug(
                            f"Linking file {file_path} to directory {rel_dir_path}",
                            job_id,
                        )
                        rel_query = "\n                        MATCH (d:Directory {path: $dir_path})\n                        MATCH (f:File {path: $file_path})\n                        MERGE (d)-[rel:CONTAINS]->(f)\n                        RETURN rel\n                        "
                        neo4j.execute_query(
                            rel_query,
                            params={"dir_path": rel_dir_path, "file_path": file_path},
                            write=True,
                        )
        ...
        overall_timing_stats = {
            ...
            "file_operations": {
                "total": file_timing_stats.get("total", 0)
                if "file_timing_stats" in locals()
                else 0,
                "metadata": file_timing_stats.get("file_metadata", 0)
                if "file_timing_stats" in locals()
                else 0,
                "node_creation": file_timing_stats.get("file_node_creation", 0)
                if "file_timing_stats" in locals()
                else 0,
                "linking": file_timing_stats.get("linking", 0)
                if "file_timing_stats" in locals()
                else 0,
                "avg_per_file": file_timing_stats.get("total", 0) / max(1, file_count)
                if "file_timing_stats" in locals() and file_count
                else 0,
            },
            "neo4j_operations": {
                "node_creation": dir_timing_stats.get("dir_node_creation", 0)
                + (
                    file_timing_stats.get("file_node_creation", 0)
                    if "file_timing_stats" in locals()
                    else 0
                ),
                "relationship_creation": dir_timing_stats.get("dir_linking", 0)
                + (
                    file_timing_stats.get("linking", 0)
                    if "file_timing_stats" in locals()
                    else 0
                ),
                "avg_operation_time": (
                    dir_timing_stats.get("dir_node_creation", 0)
        ...
        try:
            ...
            record_props = {
                ...
                "dir_count": dir_count,
                "performance": {
                    "avg_file_time": overall_timing_stats_dict["file_operations"][
                        "avg_per_file"
                    ],
                    "avg_dir_time": overall_timing_stats_dict["directory_operations"][
                        "avg_per_directory"
                    ],
                    "avg_neo4j_op_time": overall_timing_stats_dict["neo4j_operations"][
                        "avg_operation_time"
                    ],
                },
            }
        ...
        try:
            self.update_state(
                ...
                meta={
                    ...
                    "timing_stats": overall_timing_stats_dict,
                },
            )
        except Exception as e:
        ...
        try:
            self.update_state(
                state="FAILURE",
                meta={"status": StepStatus.FAILED, "error": str(e), "job_id": job_id},
            )
