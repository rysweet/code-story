...
class ConfigMetadata(BaseModel):
    ...
    json_schema: dict[str, Any] | None = Field(default=None, description='JSON Schema for validation')
...
class ConfigItem(BaseModel):
    ...
    @property
    def is_sensitive(self: Any) -> bool:
        """Check if the configuration item is sensitive."""
        return self.metadata.type == ConfigValueType.SECRET or self.metadata.permission == ConfigPermission.SENSITIVE

    def redact_if_sensitive(self: Any) -> 'ConfigItem':
...
class ConfigDump(BaseModel):
    ...
    groups: dict[ConfigSection, ConfigGroup] = Field(..., description='Configuration groups by section')
    ...
    def redact_sensitive(self: Any) -> 'ConfigDump':
        ...
        return ConfigDump(groups=redacted_groups, version=self.version, last_updated=self.last_updated)
...
class ConfigPatch(BaseModel):
    ...
    @classmethod
    def validate_items(cls: Any, v: list[ConfigPatchItem]) -> list[ConfigPatchItem]:
...
class ConfigSchemaSection(BaseModel):
    ...
    properties: dict[str, ConfigSchemaProperty] = Field(..., description='Properties in this section')
    required: list[str] = Field(default_factory=list, description='Required properties in this section')
...
class ConfigSchema(BaseModel):
    ...
    ui_schema: dict[str, Any] = Field(default_factory=dict, description='UI Schema with layout hints')

    @classmethod
    def create_from_config(cls: Any, config: ConfigDump) -> 'ConfigSchema':
        ...
        for section_name, group in config.groups.items():
            ...
            for key, item in group.items.items():
                meta = item.metadata
                prop = ConfigSchemaProperty(type=meta.type.value, title=key, description=meta.description, default=meta.default_value, read_only=meta.permission == ConfigPermission.READ_ONLY, write_only=meta.permission == ConfigPermission.SENSITIVE)
            ...
            section_schema = ConfigSchemaSection(title=section_name.value.capitalize(), description=f'Configuration for {section_name.value}', properties=section_properties, required=section_required)
        ...
        json_schema = {'$schema': 'http://json-schema.org/draft-07/schema#', 'type': 'object', 'title': 'Code Story Configuration', 'description': 'Configuration settings for Code Story service', 'properties': properties, 'required': required}
...
class ConfigValidationResult(BaseModel):
    ...
    errors: list[ConfigValidationError] = Field(default_factory=list, description='Validation errors if any')