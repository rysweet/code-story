...
class CypherQuery(BaseModel):
    ...
    query: str = Field(..., description='The Cypher query to execute', examples=['MATCH (n) RETURN n LIMIT 10'])
    parameters: dict[str, Any] = Field(default_factory=dict, description='Parameters for the Cypher query', examples=[{'name': 'John', 'limit': 10}])
    query_type: QueryType = Field(default=QueryType.READ, description='Type of query (read-only or write)')
    timeout: int | None = Field(default=30, description='Query timeout in seconds', ge=1, le=300)

    @field_validator('query')
    @classmethod
    def query_must_not_be_empty(cls: Any, v: str) -> str:
...
class QueryResult(BaseModel):
    ...
    format: QueryResultFormat = Field(default=QueryResultFormat.TABULAR, description='Format of the results')
...
class VectorQuery(BaseModel):
    ...
    query: str = Field(..., description='Natural language query or keywords for search', min_length=3)
    entity_type: EntityType | None = Field(default=EntityType.ANY, description='Type of entity to search for')
    mode: VectorSearchMode = Field(default=VectorSearchMode.SIMILARITY, description='Search mode (similarity, euclidean, hybrid)')
    limit: int = Field(default=10, description='Maximum number of results to return', ge=1, le=100)
    min_score: float = Field(default=0.5, description='Minimum similarity score (0-1)', ge=0.0, le=1.0)
...
class SearchResult(BaseModel):
    ...
    content_snippet: str | None = Field(default=None, description='Content snippet with highlighted match')
    properties: dict[str, Any] = Field(default_factory=dict, description='Additional node properties')
    path: str | None = Field(default=None, description='File path (for file/directory entities)')
...
class VectorResult(BaseModel):
    ...
    query_embedding_time_ms: int | None = Field(default=None, description='Time to generate query embedding')
...
class PathRequest(BaseModel):
    ...
    algorithm: PathAlgorithm = Field(default=PathAlgorithm.SHORTEST, description='Path finding algorithm to use')
    relationship_types: list[PathRelationshipType] | None = Field(default=None, description='Types of relationships to consider')
    direction: PathDirection = Field(default=PathDirection.BOTH, description='Direction for path traversal')
    max_depth: int = Field(default=10, description='Maximum path depth to search', ge=1, le=50)
    limit: int = Field(default=1, description='Maximum number of paths to return', ge=1, le=10)
...
class AskRequest(BaseModel):
    ...
    question: str = Field(..., description='Natural language question about the codebase', min_length=3)
    context_size: int = Field(default=5, description='Number of context items to use for answering', ge=1, le=20)
    include_code_snippets: bool = Field(default=True, description='Whether to include code snippets in the response')
    conversation_id: str | None = Field(default=None, description='ID for continued conversation context')
...
class AskAnswer(BaseModel):
    ...
    execution_time_ms: int = Field(..., description='Execution time in milliseconds')
    confidence_score: float = Field(..., description='Confidence in the answer (0-1)', ge=0.0, le=1.0)
...
class NodeFilter(BaseModel):
    ...
    max_nodes: int = Field(default=100, description='Maximum number of nodes to display initially', ge=10, le=500)
    include_orphans: bool = Field(default=False, description='Whether to include nodes with no connections')

class VisualizationRequest(BaseModel):
    """Request parameters for graph visualization."""
    type: VisualizationType = Field(default=VisualizationType.FORCE, description='Type of visualization')
    theme: VisualizationTheme = Field(default=VisualizationTheme.AUTO, description='Visualization theme')
    filter: NodeFilter | None = Field(default=None, description='Filter for nodes to include')
    focus_node_id: str | None = Field(default=None, description='Node ID to focus the visualization on')
    depth: int = Field(default=2, description='Depth of relationships to include from focus node', ge=1, le=5)

class DatabaseClearRequest(BaseModel):
    """Request to clear the database."""
    confirm: bool = Field(default=False, description='Confirmation that the database should be cleared. Must be True.')
    preserve_schema: bool = Field(default=True, description='Whether to preserve constraints and indexes.')

    @field_validator('confirm')
    @classmethod
    def confirm_must_be_true(cls: Any, v: bool) -> bool:
...
class DatabaseClearResponse(BaseModel):
    ...
    timestamp: str = Field(default_factory=lambda: time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), description='Timestamp of the operation')