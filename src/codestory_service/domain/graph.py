...
class CypherQuery(BaseModel):
    ...
    query_type: QueryType = Field(
        default=QueryType.READ, description="Type of query (read-only or write)"
    )
...
class VectorQuery(BaseModel):
    ...
    query: str = Field(
        ..., description="Natural language query or keywords for search", min_length=3
    )
    entity_type: EntityType | None = Field(
        default=EntityType.ANY, description="Type of entity to search for"
    )
    ...
    min_score: float = Field(
        default=0.5, description="Minimum similarity score (0-1)", ge=0.0, le=1.0
    )
...
class SearchResult(BaseModel):
    ...
    path: str | None = Field(
        default=None, description="File path (for file/directory entities)"
    )
...
class VectorResult(BaseModel):
    ...
    query_embedding_time_ms: int | None = Field(
        default=None, description="Time to generate query embedding"
    )
...
class PathRequest(BaseModel):
    ...
    algorithm: PathAlgorithm = Field(
        default=PathAlgorithm.SHORTEST, description="Path finding algorithm to use"
    )
    relationship_types: list[PathRelationshipType] | None = Field(
        default=None, description="Types of relationships to consider"
    )
    direction: PathDirection = Field(
        default=PathDirection.BOTH, description="Direction for path traversal"
    )
    max_depth: int = Field(
        default=10, description="Maximum path depth to search", ge=1, le=50
    )
    limit: int = Field(
        default=1, description="Maximum number of paths to return", ge=1, le=10
    )
...
class AskRequest(BaseModel):
    ...
    question: str = Field(
        ..., description="Natural language question about the codebase", min_length=3
    )
    ...
    include_code_snippets: bool = Field(
        default=True, description="Whether to include code snippets in the response"
    )
    conversation_id: str | None = Field(
        default=None, description="ID for continued conversation context"
    )
