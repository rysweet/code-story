...
def get_azure_tenant_id_from_environment() -> str | None:
    ...
    try:
        import subprocess
        result = subprocess.run(['az', 'account', 'show', '--query', 'tenantId', '-o', 'tsv'], capture_output=True, text=True, timeout=5)
...
class OpenAIAdapter:
    ...
    def __init__(self: Any, client: OpenAIClient | None=None) -> None:
        ...
        try:
            import subprocess
            result = subprocess.run(['az', 'account', 'show', '--query', 'user.name', '-o', 'tsv'], capture_output=True, text=True, timeout=5)
        ...
        try:
            ...
            try:
                self.client = client or OpenAIClient(endpoint=os.environ.get('AZURE_OPENAI__ENDPOINT'), embedding_model=os.environ.get('OPENAI__EMBEDDING_MODEL', 'text-embedding-3-small'), chat_model=os.environ.get('OPENAI__CHAT_MODEL', 'gpt-4o'), reasoning_model=os.environ.get('OPENAI__REASONING_MODEL', 'gpt-4o'), api_version=os.environ.get('AZURE_OPENAI__API_VERSION', '2025-03-01-preview'))
            ...
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=f'OpenAI authentication error: {e!s}') from e
        except Exception as e:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'Failed to initialize OpenAI client: {e!s}') from e

    async def create_embeddings(self: Any, texts: list[str]) -> list[list[float]]:
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail=f'OpenAI authentication error: {e!s}') from e
        except Exception as e:
            logger.error(f'Failed to create embeddings: {e!s}')
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'Failed to create embeddings: {e!s}') from e
    ...
    async def check_health(self: Any) -> dict[str, Any]:
        ...
        try:
            ...
            messages = [ChatMessage(role='system', content='You are a helpful assistant.'), ChatMessage(role='user', content=test_message)]
            ...
            available_models = [self.client.embedding_model, self.client.chat_model, self.client.reasoning_model]
            ...
            result = {'status': status, 'details': {'message': 'OpenAI API connection successful', 'models': [self.client.embedding_model or 'unknown', self.client.chat_model or 'unknown', self.client.reasoning_model or 'unknown'], 'api_version': getattr(self.client, 'api_version', 'latest')}}
            ...
            if '<html>' in error_message and '404 Not Found' in error_message and ('nginx' in error_message):
                ...
                return {'status': 'unhealthy', 'details': {'message': 'Azure OpenAI endpoint returned a 404 error', 'error': 'Endpoint not found or unavailable', 'current_config': {'deployment_id': deployment_id, 'endpoint': endpoint, 'api_version': api_version}, 'required_config': {'AZURE_OPENAI__DEPLOYMENT_ID': '<your-deployment-id>', 'AZURE_OPENAI__ENDPOINT': '<your-endpoint>', 'AZURE_OPENAI__API_VERSION': '<your-api-version>'}, 'suggestion': 'Set all required Azure OpenAI environment variables. No default endpoint is provided.'}}
            elif 'DefaultAzureCredential failed to retrieve a token' in error_message or 'AADSTS700003' in error_message:
                ...
                return {'status': 'unhealthy', 'details': {'error': 'Azure authentication credentials expired', 'type': 'AuthenticationError', 'tenant_id': tenant_id, 'solution': f'Run: {renewal_cmd}', 'hint': 'You can use our CLI for manual renewal: codestory service auth-renew'}}
            else:
                ...
                return {'status': 'degraded', 'details': {'message': 'OpenAI health check failed with unexpected error', 'error': error_message, 'error_type': type(e).__name__}}
