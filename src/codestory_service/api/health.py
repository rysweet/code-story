...
async def health_check(
    ...
    try:
        ...
        health_report = await asyncio.wait_for(
            _health_check_impl(neo4j, celery, openai), timeout=30
        )
        ...
        try:
            ...
            if not script_path.exists():
                ...
                class AsyncSubprocessResult:
                    def __init__(
                        self, returncode: Any, stdout: Any, stderr: Any
                    ) -> None:
...
async def _health_check_impl(
    neo4j: Neo4jAdapter, celery: CeleryAdapter, openai: OpenAIAdapter
) -> HealthReport:
    ...
    tasks = [
        ...
        check_component_health("OpenAI", openai.check_health, 10),
    ]
