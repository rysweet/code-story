...
class AuthService:
    ...
    async def login(self: Any, request: LoginRequest) -> TokenResponse:
        ...
        if not self.settings.dev_mode:
            ...
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='Login endpoint only available in development mode')
        try:
            if request.username == 'admin' and request.password == 'password':
                token = await self.validator.create_dev_token(request.username, roles=['admin', 'user'])
                return TokenResponse(access_token=token, token_type='Bearer', expires_in=self.settings.jwt_expiration, scope='api')
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'Login failed: {e!s}') from e
    ...
    async def get_user_info(self: Any, claims: dict[str, Any]) -> UserInfo:
        ...
        """
        return UserInfo(id=claims.get('sub', 'unknown'), name=claims.get('name', 'Unknown User'), email=claims.get('email'), roles=claims.get('roles', []), is_authenticated=True)

    def check_permission(self: Any, claims: dict[str, Any], required_roles: list[str]) -> bool:
