...
class IngestionService:
    ...
    async def subscribe_to_progress(self: Any, websocket: WebSocket, job_id: str) -> None:
        ...
        try:
            ...
            while True:
                try:
                    message = await asyncio.wait_for(pubsub.get_message(ignore_subscribe_messages=True), timeout=self.settings.websocket_heartbeat)
    ...
    async def start_ingestion(self: Any, request: IngestionRequest) -> IngestionStarted:
        ...
        try:
            ...
            if dependencies:
                ...
                initial_event = JobProgressEvent(job_id=job_id, step='WaitingForDependencies', status=StepStatus.PENDING, progress=0.0, overall_progress=0.0, message=f'Waiting for dependencies: {dependencies}', cpu_percent=None, memory_mb=None, timestamp=time.time())
                await self.publish_progress(job_id, initial_event)
                return IngestionStarted(job_id=job_id, status=JobStatus.PENDING, source=request.source, steps=request.steps or [], message=f'Job is waiting for dependencies: {dependencies}', eta=None)
            else:
                ingestion_started = await self.celery.start_ingestion(request)
                from codestory.ingestion_pipeline.step import StepStatus
                initial_event = JobProgressEvent(job_id=ingestion_started.job_id, step='Initializing', status=StepStatus.PENDING, progress=0.0, overall_progress=0.0, message='Preparing to start ingestion', cpu_percent=None, memory_mb=None, timestamp=ingestion_started.eta if ingestion_started.eta else None)
    ...
    async def cancel_job(self: Any, job_id: str) -> IngestionJob:
        ...
        try:
            ...
            from codestory.ingestion_pipeline.step import StepStatus
            cancel_event = JobProgressEvent(job_id=job_id, step=job.current_step if job.current_step else 'Cancelled', status=StepStatus.CANCELLED, progress=0.0, overall_progress=job.progress, message='Job was cancelled by user', cpu_percent=None, memory_mb=None, timestamp=job.updated_at if job.updated_at else None)
    ...
    async def list_jobs(self: Any, status: list[JobStatus] | None=None, limit: int=10, offset: int=0, sort_by: str='created_at', sort_order: str='desc') -> PaginatedIngestionJobs:
        ...
        try:
            ...
            total = len(jobs)
            return PaginatedIngestionJobs(items=jobs, total=total, limit=limit, offset=offset, has_more=False)
        except Exception as e:
