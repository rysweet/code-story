...
class DocumentationGrapherStep(PipelineStep):
    ...
    def run(self, repository_path: str, **config: Any) -> str:
        ...
        task = current_app.send_task('codestory_docgrapher.step.run_docgrapher', kwargs={'repository_path': repository_path, 'job_id': job_id, 'ignore_patterns': ignore_patterns, 'use_llm': use_llm, 'config': config})
...
def run_docgrapher(self, repository_path: str, job_id: str, ignore_patterns: list[str] | None=None, use_llm: bool=True, config: dict[str, Any] | None=None) -> dict[str, Any]:
    ...
    try:
        self.update_state(state='PROGRESS', meta={'progress': 0.0, 'message': 'Finding documentation files...'})
        ...
        self.update_state(state='PROGRESS', meta={'progress': 5.0, 'message': f'Found {len(doc_files)} documentation files to process'})
        ...
        self.update_state(state='PROGRESS', meta={'progress': 75.0, 'message': 'Linking documentation to code entities...'})
        knowledge_graph.link_to_code_entities()
        self.update_state(state='PROGRESS', meta={'progress': 90.0, 'message': 'Storing documentation graph in Neo4j...'})
        knowledge_graph.store_in_neo4j()
        end_time = time.time()
        duration = end_time - start_time
        stats = knowledge_graph.get_graph_stats()
        result = {'job_id': job_id, 'repository_path': repository_path, 'start_time': start_time, 'end_time': end_time, 'duration': duration, 'documents_processed': stats['documents'], 'entities_processed': stats['entities'], 'relationships_processed': stats['relationships'], 'progress': 100.0, 'status': StepStatus.COMPLETED, 'message': f"Processed {stats['documents']} documents, created {stats['entities']} entities and {stats['relationships']} relationships in {duration:.2f} seconds", 'stats': stats}
        ...
        end_time = time.time()
        duration = end_time - start_time
        return {'job_id': job_id, 'repository_path': repository_path, 'start_time': start_time, 'end_time': end_time, 'duration': duration, 'status': StepStatus.FAILED, 'error': str(e), 'message': f'DocumentationGrapher task failed: {e!s}'}
    finally:
        connector.close()