...
class ContentAnalyzer:
    ...
    def _analyze_code_block(self, content: str, language: str) -> Dict:
        ...
        if language in ["python", "py"]:
            ...
            for line in lines:
                if re.match(r'^function\s+\w+|^class\s+\w+|^const\s+\w+\s*=\s*function', line.strip()):
                    definitions.append(line.strip())
    ...
    def _extract_keywords(self, content: str) -> List[str]:
        ...
        stop_words = {
            "the", "and", "is", "in", "to", "of", "for", "with", "this", 
            "that", "it", "on", "as", "by", "an", "are", "be", "or", "from"
        }
    ...
    def _extract_function_purpose(self, content: str) -> str:
        ...
        messages = [
            ChatMessage(role=ChatRole.SYSTEM, content="You are a technical documentation analyzer."),
