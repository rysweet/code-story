...
class RstParser(Parser):
    ...
    def __init__(self):
        ...
        self.heading_pattern1 = re.compile(r'^([=\-`:\'"~^_*+#])\1{2,}\s*$\n^(.+?)$\n([=\-`:\'"~^_*+#])\3{2,}\s*$', re.MULTILINE)
        self.heading_pattern2 = re.compile(r'^(.+?)$\n([=\-`:\'"~^_*+#])\2{2,}\s*$', re.MULTILINE)
        
        # Directive patterns
        self.directive_pattern = re.compile(r'^\.\. ([a-z]+)::\s*(.*?)$\n(?:\s{3}(.*?)$)?(?:\n\s{3}(.*?)$)*', re.MULTILINE)
        
        # Code block pattern
        self.code_block_pattern = re.compile(r'^\.\. code-block::\s*(\w*)\s*$\n\s+(.+?)$(?:\n\s+.*?)*$', re.MULTILINE | re.DOTALL)
        ...
        # Image pattern
        self.image_pattern = re.compile(r'^\.\. image::\s*(.+?)$(?:\n\s+:([a-z]+):\s*(.+?)$)*', re.MULTILINE)
        
        # List pattern
        self.list_pattern = re.compile(r'(?:^\s*(?:[*+#-]|\d+[.)]) .+$\n?)+', re.MULTILINE)
    ...
    def _extract_headings_style1(self, content: str, file_path: str) -> List[DocumentationEntity]:
    ...
    def _extract_headings_style2(self, content: str, file_path: str) -> List[DocumentationEntity]:
    ...
    def _extract_links(self, content: str, file_path: str) -> List[DocumentationEntity]:
        ...
        for match in self.ext_link_pattern.finditer(content):
            ...
            entity = DocumentationEntity(
                ...
                line_number=line_number,
                metadata={
                    "url": f"#external-{text.lower().replace(' ', '-')}"
                }
            )
            entities.append(entity)
