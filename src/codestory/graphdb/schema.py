...
def initialize_schema(connector, force: bool = False) -> None:
    ...
    for query in schema_queries:
        try:
            ...
            if "already exists" in error_message or "equivalentschemarulealreadyexists" in error_message:
...
def verify_schema(connector) -> Dict[str, Dict[str, bool]]:
    ...
    try:
        ...
        existing_constraints = [
            record["name"] for record in constraints_result
        ]
        
        # Check indexes
        indexes_result = connector.execute_query("SHOW INDEXES")
        existing_indexes = [
            record["name"] for record in indexes_result
        ]
        ...
        for constraint in schema_elements["constraints"]:
            ...
            verification_results["constraints"][constraint_name] = constraint_name in existing_constraints
