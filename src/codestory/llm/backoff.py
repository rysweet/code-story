...
def get_retry_after(retry_state: RetryCallState) -> float | None:
    ...
    if exception is not None and hasattr(exception, 'retry_after') and (exception.retry_after is not None):
        return float(min(exception.retry_after, 60))
    return None
...
def before_retry_callback(retry_state: RetryCallState) -> None:
    ...
    logger.warning(f'Retrying {operation} request to model {model} after error: {exception!s}. Attempt {attempt}, waiting {wait_time:.2f} seconds...')
    operation_type = retry_state.kwargs.get('_operation_type')
    if operation_type:
        record_retry(operation_type, model)

def retry_on_openai_errors(max_retries: int=5, retry_backoff_factor: float=2.0, operation_type: OperationType | None=None) -> Callable[[F], F]:
    ...
    def decorator(func: F) -> F:
        ...
        @retry(stop=stop_after_attempt(max_retries), wait=wait_exponential(multiplier=retry_backoff_factor, min=1, max=60), retry=retry_if_exception_type((RateLimitError, ServiceUnavailableError, TimeoutError, openai.RateLimitError, openai.APIConnectionError, openai.APITimeoutError)), before_sleep=before_sleep_log(logger, logging.WARNING), after=before_retry_callback)
        def wrapper(*args: Any, **kwargs: Any) -> None:
            ...
            try:
                ...
                if hasattr(e, 'headers') and e.headers:
                    ...
                    if retry_after:
                        try:
                            ...
                            retry_after = None
                raise RateLimitError(f'Rate limit exceeded: {e!s}', retry_after=retry_after, cause=e) from e
                ...
                if 'maximum context length' in str(e).lower():
                    ...
                    raise ContextLengthError(f'Input context length exceeded model maximum: {e!s}', cause=e) from e
...
def retry_on_openai_errors_async(max_retries: int=5, retry_backoff_factor: float=2.0, operation_type: OperationType | None=None) -> Callable[[F], F]:
