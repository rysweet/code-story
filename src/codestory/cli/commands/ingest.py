...
def is_repo_mounted(repo_path, console=None):
    ...
    try:
        ...
        for service in services_to_check:
            ...
            result = subprocess.run(
                ["docker", "exec", service, "bash", "-c", f"test -d {container_path} && ls -la {container_path} | wc -l | grep -v '^[[:space:]]*2[[:space:]]*$'"],
                capture_output=True,
                text=True,
                check=False
            )
            ...
            for test_file in test_files:
                ...
                if os.path.exists(local_test_file):
                    file_check = subprocess.run(
                        ["docker", "exec", service, "test", "-f", f"{container_path}/{test_file}", "&&", "echo", "exists"],
                        shell=True,
                        capture_output=True,
                        text=True,
                        check=False
            ...
            try:
                ...
                if service in service_check.stdout:
                    ...
                    dir_check = subprocess.run(
                        ["docker", "exec", service, "test", "-d", container_path, "&&", "echo", "exists"],
                        shell=True,
                        capture_output=True,
                        text=True,
                        check=False
