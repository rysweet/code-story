...
def setup_repository_mount(
    repo_path: str, console: Optional[Any] = None, force_remount: bool = False
) -> bool:
...
def start_ingestion(
    ...
    Guard: If called programmatically, ctx must be a click.Context instance with .obj containing required objects.
    Do not treat ctx as iterable. Use ctx.obj["console"], ctx.obj["client"], etc.

    When container=True and auto_mount=False, and the repository is not mounted in the container,
    this function raises click.ClickException("Repository not mounted in container").
    This ensures programmatic and CLI calls both validate mount status and propagate errors for test capture.
    ...
    # Guard: If running in container mode and auto_mount is False, verify mount and raise if missing.
    if is_container and not auto_mount:
        if not is_repo_mounted(local_path, console):
            # Print error to console for test capture before raising
            console.print("[red]Repository not mounted in container[/]")
            raise click.ClickException("Repository not mounted in container")
            ...
            if isinstance(eta, str):
                try:
                    ...
                    try:
                        ...
                        console.print(
                            f"[yellow]Warning: Could not parse --eta value '{eta}'[/]"
                        )
...
def start_ingestion_cli(
    ...
    """CLI entrypoint for start_ingestion."""
