        ...
        try:
            ...
            if 'No such command' in error_msg and 'Did you mean' not in error_msg:
                ...
                if cmd_name:
                    ...
                    similar: List[str] = [cmd for cmd in commands if cmd.startswith(cmd_name[:1]) or cmd_name.lower() in cmd.lower()]
                    if similar:
                        error_msg += f"\n\nDid you mean one of these?\n    {', '.join(similar)}"
            click.echo(f'Error: {error_msg}', err=True)
            ctx.exit(e.exit_code)

@click.group(cls=CodeStoryCommandGroup, help='[bold]Code Story[/bold] - A tool for exploring and documenting codebases.', invoke_without_command=True, max_suggestions=5, cutoff=0.5)
@click.version_option()
@click.option('--service-url', help='URL of the Code Story service.', envvar='CODESTORY_SERVICE_URL')
@click.option('--api-key', help='API key for authentication.', envvar='CODESTORY_API_KEY')
@click.pass_context
def app(ctx: click.Context, service_url: Optional[str]=None, api_key: Optional[str]=None) -> None:
    ...
    client = ServiceClient(base_url=base_url, api_key=api_key, console=console, settings=settings)
