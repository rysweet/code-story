...
class ServiceClient:
    ...
    def check_service_health(
        self, auto_fix: bool = False, timeout: int | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                ...
                return health_data[no-any-return]
            ...
            try:
                ...
                if response.status_code < 500:  # Accept any non-server error response
                    ...
                    self.console.print(
                        f"[dim]Health check endpoint not available: {e!s}[/]",
                        style="dim",
                    )
                    return resp_data
                ...
                if (
                    ...
                    if renewal_attempted:
                        if renewal_success:
                            ...
                            self.console.print(
                                "3. Try restarting the service: codestory service restart"
                            )
                        else:
                            self.console.print(
                                "[yellow]Automatic renewal was attempted but failed.[/yellow]"
                            )
                    ...
                    health_data["components"]["openai"]["details"][
                        "user_message"
                    ] = "Azure authentication credentials expired. See instructions for renewal."
    ...
    def start_ingestion(
        ...
        if not is_container_path:
            ...
            if os.path.exists(repo_config_path):
                self.console.print(
                    f"[dim]Repository config file found at [cyan]{repo_config_path}[/][/]",
                    style="dim",
                )
        ...
        self.console.print(
            f"Starting ingestion for repository at [cyan]{abs_repository_path}[/]",
            style="dim",
        )
        self.console.print(
            f"Repository directory exists: [cyan]{os.path.isdir(abs_repository_path)}[/]",
            style="dim",
        )
    ...
    def list_ingestion_jobs(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                if "items" in data:
                    return data["items"][no-any-return]
                # Legacy format with 'jobs' field
                elif "jobs" in data:
                    return data["jobs"][no-any-return]
    ...
    def execute_query(
        ...
        if parameters:
            data["parameters"] = parameters  # TODO: Fix type compatibility
            ...
            for endpoint in endpoints_to_try:
                try:
                    ...
                    if endpoint.startswith("/v1/") and self.base_url.endswith("/v1"):
                        ...
                        temp_client = httpx.Client(
                            base_url=base_url_without_v1, timeout=30.0, headers=headers
                        )
                        actual_endpoint = endpoint
                        client = temp_client
                    else:
                    ...
                    # Make the request
                    self.console.print(
                        f"Trying visualization endpoint: [cyan]{actual_endpoint}[/]",
                        style="dim",
                    )
                    if params:
