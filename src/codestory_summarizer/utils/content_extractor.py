...
class ContentExtractor:
    ...
    def _extract_repository_content(self, node: NodeData) -> dict[str, str | list[str]]:
        ...
        context = [f'Repository: {repo_name}', f'Path: {repo_path}', f'Contains {dir_count} directories and {file_count} files', f"Top-level directories: {', '.join(top_level_dirs)}"]
        if readme_content:
    ...
    def _extract_directory_content(self, node: NodeData) -> dict[str, str | list[str]]:
        ...
        context = [f'Directory: {dir_name}', f'Path: {dir_path}', f'Contains {len(files)} files and {len(subdirs)} subdirectories']
        if file_list:
    ...
    def _extract_file_content(self, node: NodeData) -> dict[str, str | list[str]]:
        ...
        binary_extensions = {'png', 'jpg', 'jpeg', 'gif', 'ico', 'svg', 'pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'zip', 'tar', 'gz', 'tgz', 'rar', '7z', 'exe', 'dll', 'so', 'dylib', 'o', 'obj', 'pyc', 'pyo', 'pyd'}
        if file_extension in binary_extensions:
            return {'content': f'Binary file: {file_path}', 'context': [f'Binary file of type: {file_extension}']}
        context = [f'File: {file_name}', f'Path: {file_path}', f'Type: {file_extension}']
        if self.repository_path and file_path:
            ...
            content_results = self.connector.execute_query(content_query, params={'file_id': int(node.id)})
    ...
    def _extract_class_content(self, node: NodeData) -> dict[str, str | list[str]]:
        ...
        if file_path and self.repository_path:
            ...
            try:
                ...
                for i, line in enumerate(lines):
                    ...
                    if class_start != -1:
                        ...
                        if '}' in line and brace_level == 0:
                            class_end = i
                            break
                        if i > class_start and line.strip() == '' and (i + 1 < len(lines)) and (not lines[i + 1].startswith(' ')):
                            class_end = i
                            break
                if class_start != -1:
                    if class_end == -1:
                ...
                logger.warning(f'Error extracting class content for {class_name} from {absolute_path}: {e}')
    ...
    def _extract_function_content(self, node: NodeData) -> dict[str, str | list[str]]:
        ...
        container_results = self.connector.execute_query(container_query, params={'func_id': int(node.id)})
        ...
        if 'Class' in container_labels:
            ...
            file_results = self.connector.execute_query(file_query, params={'func_id': int(node.id)})
            file_result = file_results[0] if file_results and len(file_results) > 0 else None
            if file_result:
        ...
        if file_path and self.repository_path is not None:
            ...
            try:
                ...
                brace_level = 0
                for i, line in enumerate(lines):
                    if (f'def {func_name}' in line or f'function {func_name}' in line or f'{func_name} = function' in line or (f'{func_name}(' in line)) and func_start == -1:
                        func_start = i
                    if func_start != -1:
                        brace_level += line.count('{') - line.count('}')
                        if '}' in line and brace_level == 0:
                            func_end = i
                            break
                        if i > func_start and line.strip() == '' and (i + 1 < len(lines)) and (not lines[i + 1].startswith(' ')):
                            func_end = i
                            break
                if func_start != -1:
                    if func_end == -1:
                ...
                logger.warning(f'Error extracting function content for {func_name} from {absolute_path}: {e}')
