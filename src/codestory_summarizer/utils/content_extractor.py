...
class ContentExtractor:
    ...
    def __init__(self, connector: Neo4jConnector, repository_path: Optional[str] = None):
    ...
    def _extract_repository_content(self, node: NodeData) -> Dict[str, Union[str, List[str]]]:
        ...
        dir_result = self.connector.run_query(
            dir_query,
            parameters={"repo_id": int(node.id)},
            fetch_one=True
        )
    ...
    def _extract_directory_content(self, node: NodeData) -> Dict[str, Union[str, List[str]]]:
    ...
    def _extract_file_content(self, node: NodeData) -> Dict[str, Union[str, List[str]]]:
        ...
        if self.repository_path:
            ...
            content_result = self.connector.run_query(
                content_query,
                parameters={"file_id": int(node.id)},
                fetch_one=True
            )
            
            content = content_result["content"] if content_result and "content" in content_result else ""
    ...
    def _extract_class_content(self, node: NodeData) -> Dict[str, Union[str, List[str]]]:
        ...
        file_result = self.connector.run_query(
            file_query,
            parameters={"class_id": int(node.id)},
            fetch_one=True
        )
    ...
    def _extract_function_content(self, node: NodeData) -> Dict[str, Union[str, List[str]]]:
        ...
        container_result = self.connector.run_query(
            container_query,
            parameters={"func_id": int(node.id)},
            fetch_one=True
        )
        
        container_id = str(container_result["container_id"]) if container_result else None
        container_labels = container_result["container_labels"] if container_result else []
            ...
            try:
                ...
                for i, line in enumerate(lines):
                    if (f"def {func_name}" in line or f"function {func_name}" in line or 
                        f"{func_name} = function" in line or f"{func_name}(" in line) and func_start == -1:
                        func_start = i
